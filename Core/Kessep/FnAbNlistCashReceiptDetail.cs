// Program: FN_AB_NLIST_CASH_RECEIPT_DETAIL, ID: 371876095, model: 746.
// Short name: SWE02271
using System;
...
/// <summary>
/// <para>
/// A program: FN_AB_NLIST_CASH_RECEIPT_DETAIL.
/// </para>
/// <para>
...
/// </summary>
[Serializable]
public partial class FnAbNlistCashReceiptDetail: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_AB_NLIST_CASH_RECEIPT_DETAIL program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnAbNlistCashReceiptDetail(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnAbNlistCashReceiptDetail.
  /// </summary>
  public FnAbNlistCashReceiptDetail(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // 12/01/98  S Newman	Rewrote cab to remove code causing duplicate displays.
    // Revised reads to process
    // 			three filters with If Statements rather than qualified reads.  Revised
    // reads to
    // 			check for only NON REIP payments.  This action block will only be used
    // when the
    // 			Pay History Indicator on CRDL = 'N'
    ...
    ReadCashReceiptDetailCashReceiptCashReceiptDetailStatHistory1()
  {
    return ReadEach(
      ...
      (db, reader) =>
      {
        ...
        entities.CashReceiptDetailStatus.Code = db.GetString(reader, 22);
        entities.CashReceiptSourceType.SystemGeneratedIdentifier =
          db.GetInt32(reader, 23);
        entities.IdOnlyCashReceiptEvent.CstIdentifier = db.GetInt32(reader, 23);
        entities.CashReceiptSourceType.Code = db.GetString(reader, 24);
        entities.CashReceiptSourceType.EffectiveDate = db.GetDate(reader, 25);
        entities.CashReceiptSourceType.DiscontinueDate =
          db.GetNullableDate(reader, 26);
        entities.IdOnlyCashReceiptEvent.ReceivedDate = db.GetDate(reader, 27);
        entities.IdOnlyCashReceiptType.Code = db.GetString(reader, 28);
    ...
    ReadCashReceiptDetailCashReceiptCashReceiptDetailStatHistory5()
  {
    return ReadEach(
      ...
      (db, reader) =>
      {
        ...
        entities.CashReceiptDetailStatHistory.DiscontinueDate =
          db.GetNullableDate(reader, 21);
        entities.IdOnlyCashReceiptEvent.ReceivedDate = db.GetDate(reader, 22);
        entities.IdOnlyCashReceiptType.Code = db.GetString(reader, 23);
        entities.CashReceipt.Populated = true;
        entities.CashReceiptDetail.Populated = true;
        entities.CashReceiptDetailStatHistory.Populated = true;
        entities.IdOnlyCashReceiptEvent.Populated = true;
