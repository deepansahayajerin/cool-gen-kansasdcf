...
Test:
    ...
    foreach(var item in ReadCaseCaseRoleCaseRole3())
    {
    ...
    if (IsEmpty(local.Case1.Number))
    {
      ...
      foreach(var item in ReadCaseCaseRoleCaseRole1())
      {
        if (!ReadCaseAssignment())
        {
          // -- Case was not open during the report period.  Look for another 
    ...
    if (IsEmpty(local.Case1.Number))
    {
      ...
      foreach(var item in ReadCaseCaseRoleCaseRole2())
      {
        if (!ReadCaseAssignment())
        {
          // -- Case was not open during the report period.  Look for another 
    ...
    if (IsEmpty(local.Case1.Number))
    {
      ...
      local.EabReportSend.RptDetail =
        ...
        entities.ApCsePerson.Number + " Supported " + entities
        .SupportedCsePerson.Number + ".  Debt Due Date " + local
        .TextWorkArea.Text10;
      UseCabErrorReport();
    ...
    else
    {
      ...
      local.EabReportSend.RptDetail =
        ...
        local.Case1.Number + " in office " + NumberToString
  ...
  private IEnumerable<bool> ReadCaseCaseRoleCaseRole1()
  {
    entities.Case1.Populated = false;
    entities.ApCaseRole.Populated = false;
    entities.SupportedCaseRole.Populated = false;

    return ReadEach("ReadCaseCaseRoleCaseRole1",
      (db, command) =>
      {
        db.SetNullableDate(
          command, "endDate", local.CaseDate.Date.GetValueOrDefault());
  ...
  private IEnumerable<bool> ReadCaseCaseRoleCaseRole2()
  {
    entities.Case1.Populated = false;
    entities.ApCaseRole.Populated = false;
    entities.SupportedCaseRole.Populated = false;

    return ReadEach("ReadCaseCaseRoleCaseRole2",
      (db, command) =>
      {
        db.SetNullableDate(
          command, "startDate", local.CaseDate.Date.GetValueOrDefault());
        db.SetString(command, "cspNumber1", entities.ApCsePerson.Number);
        db.SetString(command, "cspNumber2", entities.SupportedCsePerson.Number);
      },
      (db, reader) =>
  ...
  private IEnumerable<bool> ReadCaseCaseRoleCaseRole3()
  {
    entities.Case1.Populated = false;
    entities.ApCaseRole.Populated = false;
    entities.SupportedCaseRole.Populated = false;

    return ReadEach("ReadCaseCaseRoleCaseRole3",
      (db, command) =>
      {
        db.SetNullableDate(
          command, "startDate", local.CaseDate.Date.GetValueOrDefault());
  ...
  private bool ReadTribunal()
  {
    ...
    return Read("ReadTribunal",
      (db, command) =>
      {
        db.SetInt32(
          command, "identifier",
          import.PersistentLegalAction.TrbId.GetValueOrDefault());
      },
      (db, reader) =>
      {
...
#region Structures
  ...
  public class Import
  {
    ...
    /// </summary>
    [JsonPropertyName("persistentLegalAction")]
