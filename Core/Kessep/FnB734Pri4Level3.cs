...
#region Implementation
  ...
  public void Run()
  {
    ...
    if (AsChar(local.ProcessCsDue.Flag) == 'Y')
    {
      ...
      // period.
      foreach(var item in ReadDebtDebtDetailCsePersonCsePersonObligation())
      {
    ...
    if (AsChar(local.ProcessCsCollected.Flag) == 'Y')
    {
      ...
      foreach(var item in ReadCollectionCsePersonDebtDetailCsePerson())
      {
        if (!Equal(entities.ApCsePerson.Number, local.PrevAp.Number))
        {
          // ------------------------------------------------------------------------------
    ...
    foreach(var item in ReadDashboardStagingPriority3())
    {
      ...
      // 02/03/14 GVandy  CQ42584  Correct Divide by Zero error.
      if (Equal(entities.DashboardStagingPriority4.CsDueAmt, 0))
      {
        local.DashboardStagingPriority4.PayingCaseInd = "N";
      }
      else if (entities.DashboardStagingPriority4.CsCollectedAmt.
        GetValueOrDefault() / entities
        .DashboardStagingPriority4.CsDueAmt.GetValueOrDefault() >= 0.75M)
      {
  ...
  private IEnumerable<bool> ReadCollectionCsePersonDebtDetailCsePerson()
  {
    ...
    return ReadEach("ReadCollectionCsePersonDebtDetailCsePerson",
      (db, command) =>
      {
  ...
  private IEnumerable<bool> ReadDebtDebtDetailCsePersonCsePersonObligation()
  {
    ...
    return ReadEach("ReadDebtDebtDetailCsePersonCsePersonObligation",
      (db, command) =>
      {
        db.SetDate(
  ...
  {
    var csCollectedAmt =
      entities.DashboardStagingPriority4.CsCollectedAmt.GetValueOrDefault() +
  ...
  {
    var csDueAmt =
      entities.DashboardStagingPriority4.CsDueAmt.GetValueOrDefault() +
      local.Debt.Amount;
...
#region Structures
  ...
  public class Local
  {
    ...
    /// </summary>
    [JsonPropertyName("processCsCollected")]
    ...
    /// </summary>
    [JsonPropertyName("tbdLocalReportEndDate")]
