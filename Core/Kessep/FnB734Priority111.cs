    ...
    foreach(var item in ReadCseOrganization())
    {
      ...
      foreach(var item1 in ReadOffice())
      {
        if (ReadOfficeStaffing())
        {
          local.Local1.Update.G.FullTimeEquivalent =
            local.Local1.Item.G.FullTimeEquivalent.GetValueOrDefault() + entities
      ...
      if (local.Contractor.Count < 1)
      {
        ...
        local.Contractor.Update.GlocalContractorOfficeStaffing.
          ...
          .Local1.Item.G.FullTimeEquivalent.GetValueOrDefault();
      }
      else
      {
          ...
          .Contractor.Count; ++local.Contractor.Index)
        {
              ...
              ContractorNumber))
          {
            local.Contractor.Update.GlocalContractorOfficeStaffing.
              FullTimeEquivalent =
                local.Local1.Item.G.FullTimeEquivalent.GetValueOrDefault() + local
    ...
    foreach(var item in ReadDashboardStagingPriority1())
    {
      ...
      switch(TrimEnd(entities.DashboardStagingPriority12.ReportLevel))
      {
        ...
        case "JD":
          ...
          local.Temp.CasesPerFteDenominator =
            local.Local1.Item.G.FullTimeEquivalent.GetValueOrDefault();
...
Test:
      ...
      if (local.Temp.CasesPerFteDenominator.GetValueOrDefault() == 0)
      {
      ...
      else
      {
        local.Temp.CasesPerFteAverage =
          Math.Round(
            local.Temp.CasesPerFteNumerator.GetValueOrDefault() /
          local.Temp.CasesPerFteDenominator.GetValueOrDefault(), 2,
    ...
    foreach(var item in ReadDashboardStagingPriority2())
    {
      ...
      if (Equal(entities.DashboardStagingPriority12.CasesPerFteAverage,
        local.PrevRank.CasesPerFteAverage.GetValueOrDefault()))
      {
  ...
  private void UpdateDashboardStagingPriority1()
  {
    ...
    var casesPerFteDenominator =
      local.Temp.CasesPerFteDenominator.GetValueOrDefault();
    var casesPerFteAverage = local.Temp.CasesPerFteAverage.GetValueOrDefault();
  ...
  {
    var casesPerFteRank = local.Temp.CasesPerFteRank.GetValueOrDefault();
