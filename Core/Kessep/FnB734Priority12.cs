...
ReadEach1:
      ...
      (import.ProgramCheckpointRestart.RestartInfo, 81, 8, "1-02    "))
    {
        ...
        IsEmpty(local.RestartNumerator.Number))
      {
        ...
        foreach(var item in ReadDashboardStagingPriority5())
        {
      ...
      (local.RestartSection.Text1) == 'N')
    {
      ...
      foreach(var item in ReadCsePerson2())
      {
        ...
        // -------------------------------------------------------------------
        foreach(var item1 in ReadCaseRoleCase1())
        {
          // -----------------------------------------------------
          ...
          // --------------------------------------------------------------------------
          foreach(var item2 in ReadGoodCauseCaseRole1())
          {
            // ---------------------------------------------------------------------
          ...
          if (local.ChildCaseCount.Count == 1)
          {
            ...
            local.Statewide.PepNumerator =
              local.Statewide.PepNumerator.GetValueOrDefault() + 1;
          }
          ...
          if (!IsEmpty(local.DashboardAuditData.JudicialDistrict))
          {
            ...
            if (local.ChildInJds.Item.GlocalChildInJd.Count == 1)
            {
              ...
              local.Local1.Update.G.PepNumerator =
                local.Local1.Item.G.PepNumerator.GetValueOrDefault() + 1;
...
ReadEach2:
    ...
    local.EabReportSend.RptDetail =
      ...
      (local.Ocse157Data.FiscalYear.GetValueOrDefault(), 12, 4) + " Run Number " +
      NumberToString(local.Ocse157Data.RunNumber.GetValueOrDefault(), 12, 4) + " Line 5 count " +
      NumberToString(local.Ocse157Data.Number.GetValueOrDefault(), 4, 12);
    UseCabControlReport();
    ...
    foreach(var item in ReadOcse157Verification())
    {
      ...
      // Read CH case roles that are active at some point during FY.
      // -------------------------------------------------------------------
      foreach(var item1 in ReadCaseRoleCase2())
      {
        // -----------------------------------------------------
        // Skip case if there is no juridiction.
        // -----------------------------------------------------
        ...
        // records where there is no CO created after the GC record.
        // --------------------------------------------------------------------------
        foreach(var item2 in ReadGoodCauseCaseRole2())
        {
          // ---------------------------------------------------------------------
          // Ensure there is no CO record that is created after the GC
          // record but before FY end.
        ...
        if (local.ChildCaseCount.Count == 1)
        {
          ...
          local.Statewide.PepDenominator =
            local.Statewide.PepDenominator.GetValueOrDefault() + 1;
        }
        ...
        if (!IsEmpty(local.DashboardAuditData.JudicialDistrict))
        {
          ...
          if (local.ChildInJds.Item.GlocalChildInJd.Count == 1)
          {
            ...
            local.Local1.Update.G.PepDenominator =
              local.Local1.Item.G.PepDenominator.GetValueOrDefault() + 1;
...
ReadEach3:
    ...
    local.EabReportSend.RptDetail =
      "Priority 1-2 Denominator using OCSE157 Fiscal Year " + NumberToString
      (local.Ocse157Data.FiscalYear.GetValueOrDefault(), 12, 4) + " Run Number " +
      NumberToString(local.Ocse157Data.RunNumber.GetValueOrDefault(), 12, 4) + " Line 5 children found " +
      NumberToString(local.Ocse157Verifi.Count, 4, 12);
    UseCabControlReport();
        ...
        .Contractor.Count; ++local.Contractor.Index)
      {
          ...
          local.Contractor.Item.Gcontractor.ContractorNumber))
        {
          local.Contractor.Update.Gcontractor.PepDenominator =
            local.Contractor.Item.Gcontractor.PepDenominator.
              GetValueOrDefault() + local
            .Local1.Item.G.PepDenominator.GetValueOrDefault();
          local.Contractor.Update.Gcontractor.PepNumerator =
            local.Contractor.Item.Gcontractor.PepNumerator.GetValueOrDefault() +
            local.Local1.Item.G.PepNumerator.GetValueOrDefault();
        ...
        .Contractor.Count; ++local.Contractor.Index)
      {
          ...
          local.Contractor1.Code))
        {
          local.Contractor.Update.Gcontractor.PrevYrPepNumerator =
            local.Contractor.Item.Gcontractor.PrevYrPepNumerator.
              GetValueOrDefault() + entities
            .PreviousYear.PepNumerator.GetValueOrDefault();
          local.Contractor.Update.Gcontractor.PrevYrPepDenominator =
            local.Contractor.Item.Gcontractor.PrevYrPepDenominator.
              GetValueOrDefault() + entities
            .PreviousYear.PepDenominator.GetValueOrDefault();
...
ReadEach4:
    ...
    foreach(var item in ReadDashboardStagingPriority7())
    {
      ...
      if (local.Temp.PepDenominator.GetValueOrDefault() == 0)
      {
      ...
      else
      {
        local.Temp.PepPercent =
          Math.Round((decimal)local.Temp.PepNumerator.GetValueOrDefault() /
          local.Temp.PepDenominator.GetValueOrDefault(), 3,
      ...
      if (local.Temp.PrevYrPepDenominator.GetValueOrDefault() == 0)
      {
      ...
      else
      {
        local.Temp.PrevYrPepPercent =
          Math.Round((decimal)local.Temp.PrevYrPepNumerator.
            GetValueOrDefault() /
          local.Temp.PrevYrPepDenominator.GetValueOrDefault(), 3,
          MidpointRounding.AwayFromZero);
      }

      // -- Calculate percent change between Current Year PEP percent and 
      // Previous Year PEP percent.
      if (local.Temp.PrevYrPepPercent.GetValueOrDefault() == 0)
      {
      ...
      else
      {
        local.Temp.PercentChgBetweenYrsPep =
          Math.Round((
            local.Temp.PepPercent.GetValueOrDefault() - local
          .Temp.PrevYrPepPercent.GetValueOrDefault()) /
          local.Temp.PrevYrPepPercent.GetValueOrDefault(), 3,
          MidpointRounding.AwayFromZero);
      }
    ...
    foreach(var item in ReadDashboardStagingPriority8())
    {
      ...
      if (Equal(entities.DashboardStagingPriority12.PepPercent,
        local.PrevRank.PepPercent.GetValueOrDefault()))
      {
  ...
  private void CreateDashboardStagingPriority1()
  {
    ...
    var pepNumerator =
      local.Contractor.Item.Gcontractor.PepNumerator.GetValueOrDefault();
    var pepDenominator =
      local.Contractor.Item.Gcontractor.PepDenominator.GetValueOrDefault();
    var pepPercent =
      local.Contractor.Item.Gcontractor.PepPercent.GetValueOrDefault();
    var pepRank = local.Contractor.Item.Gcontractor.PepRank.GetValueOrDefault();
    var contractorNumber =
      local.Contractor.Item.Gcontractor.ContractorNumber ?? "";
    var prevYrPepNumerator =
      local.Contractor.Item.Gcontractor.PrevYrPepNumerator.GetValueOrDefault();
    var prevYrPepDenominator =
      local.Contractor.Item.Gcontractor.PrevYrPepDenominator.
        GetValueOrDefault();
    var prevYrPepPercent =
      local.Contractor.Item.Gcontractor.PrevYrPepPercent.GetValueOrDefault();
    var percentChgBetweenYrsPep =
      local.Contractor.Item.Gcontractor.PercentChgBetweenYrsPep.
        GetValueOrDefault();
  ...
  private void CreateDashboardStagingPriority2()
  {
    ...
    var param = 0M;
    var pepNumerator = local.Local1.Item.G.PepNumerator.GetValueOrDefault();
    var pepDenominator = local.Local1.Item.G.PepDenominator.GetValueOrDefault();
    var pepPercent = local.Local1.Item.G.PepPercent.GetValueOrDefault();
    var pepRank = local.Local1.Item.G.PepRank.GetValueOrDefault();
  ...
  private void CreateDashboardStagingPriority3()
  {
    ...
    var param = 0M;
    var pepNumerator = local.Statewide.PepNumerator.GetValueOrDefault();
    var pepDenominator = local.Statewide.PepDenominator.GetValueOrDefault();
    var pepPercent = local.Statewide.PepPercent.GetValueOrDefault();
    var pepRank = local.Statewide.PepRank.GetValueOrDefault();
  ...
  private bool ReadCaseAssignment2()
  {
    ...
    return Read("ReadCaseAssignment2",
      (db, command) =>
      {
        ...
        db.SetDate(
          command, "effectiveDate",
          local.PriorFiscalYrEndDate.Date.GetValueOrDefault());
        db.SetNullableDate(
          command, "discontinueDate",
          local.PriorFiscalYrStartDate.Date.GetValueOrDefault());
      },
      (db, reader) =>
      {
        entities.CaseAssignment.EffectiveDate = db.GetDate(reader, 0);
  ...
  private bool ReadCsePerson1()
  {
    ...
    return Read("ReadCsePerson1",
      ...
      {
        db.SetString(
          command, "numb", entities.Ocse157Verification.SuppPersonNumber ?? ""
          );
      },
      (db, reader) =>
      {
  ...
  private bool ReadGoodCause2()
  {
    ...
    return Read("ReadGoodCause2",
      (db, command) =>
      {
        ...
        db.SetDateTime(
          command, "createdTimestamp2",
          local.PriorFiscalYrEndDate.Timestamp.GetValueOrDefault());
      },
      (db, reader) =>
      {
        entities.Next.Code = db.GetNullableString(reader, 0);
  ...
  private IEnumerable<bool> ReadGoodCauseCaseRole1()
  {
    entities.Ar.Populated = false;
    entities.GoodCause.Populated = false;

    return ReadEach("ReadGoodCauseCaseRole1",
      (db, command) =>
      {
  ...
  private IEnumerable<bool> ReadGoodCauseCaseRole2()
  {
    entities.Ar.Populated = false;
    entities.GoodCause.Populated = false;

    return ReadEach("ReadGoodCauseCaseRole2",
      (db, command) =>
      {
        db.SetString(command, "casNumber", entities.Case1.Number);
        db.SetNullableDate(
  ...
  private bool ReadOcse157Data()
  {
    ...
    return Read("ReadOcse157Data",
      (db, command) =>
      {
        db.SetNullableInt32(
          command, "fiscalYear",
          local.Ocse157Data.FiscalYear.GetValueOrDefault());
  ...
  private IEnumerable<bool> ReadOcse157Verification()
  {
    ...
    return ReadEach("ReadOcse157Verification",
      (db, command) =>
      {
        ...
        db.SetNullableInt32(
          command, "runNumber",
          local.Ocse157Data.RunNumber.GetValueOrDefault());
        db.SetNullableString(
          command, "lineNumber", local.Ocse157Data.LineNumber ?? "");
        db.SetNullableString(
  ...
  {
    var pepPercent = local.Temp.PepPercent.GetValueOrDefault();
    var prevYrPepNumerator = local.Temp.PrevYrPepNumerator.GetValueOrDefault();
    var prevYrPepDenominator =
      local.Temp.PrevYrPepDenominator.GetValueOrDefault();
    var prevYrPepPercent = local.Temp.PrevYrPepPercent.GetValueOrDefault();
    var percentChgBetweenYrsPep =
      local.Temp.PercentChgBetweenYrsPep.GetValueOrDefault();
  ...
  {
    var pepRank = local.Temp.PepRank.GetValueOrDefault();
...
#region Structures
  ...
  public class Import
  {
    ...
    /// </summary>
    [JsonPropertyName("fiscalYearStartDate")]
    ...
    /// </summary>
    [JsonPropertyName("fiscalYearEndDate")]
  ...
  public class Local
  {
    ...
    public class ChildInJdsGroup
    {
      ...
      /// </summary>
      [JsonPropertyName("glocalChildInJd")]
    ...
    /// </summary>
    [JsonPropertyName("restartDenominator")]
    ...
    /// </summary>
    [JsonPropertyName("restartNumerator")]
    ...
    /// </summary>
    [JsonPropertyName("priorFiscalYrStartDate")]
    ...
    /// </summary>
    [JsonPropertyName("priorFiscalYrEndDate")]
    ...
    /// </summary>
    [JsonPropertyName("ocse157Verifi")]
    ...
    /// </summary>
    [JsonPropertyName("childCaseCount")]
