...
#region Implementation
  ...
  public void Run()
  {
    ...
    // (for locate purposes) and to a case (to report to a caseworker/office/
    // etc).
      ...
      ; ++local.ReportingPeriod.Index)
    {
      ...
      foreach(var item in ReadLegalActionServiceProcess())
      {
            ...
            (entities.LegalReferral.ReferralReason4, "EST"))
          {
            ...
            if (!IsEmpty(local.DashboardAuditData.JudicialDistrict))
            {
              ...
              local.Local1.Update.G.DaysToReturnOfSrvcNumer =
                local.Local1.Item.G.DaysToReturnOfSrvcNumer.
                  GetValueOrDefault() + local
                .DashboardAuditData.DaysReported.GetValueOrDefault();
              local.Local1.Update.G.DaysToReturnOfServiceDenom =
                local.Local1.Item.G.DaysToReturnOfServiceDenom.
                  GetValueOrDefault() + 1;

              if (local.Local1.Item.G.DaysToReturnOfServiceDenom.
                GetValueOrDefault() > 0 && local
                .Local1.Item.G.DaysToReturnOfSrvcNumer.GetValueOrDefault() > 0)
              {
                local.Local1.Update.G.DaysToReturnOfServiceAvg =
                  (decimal)local.Local1.Item.G.DaysToReturnOfSrvcNumer.
                    GetValueOrDefault() / local
                  .Local1.Item.G.DaysToReturnOfServiceDenom.GetValueOrDefault();
          ...
          .ProgramCheckpointRestart.UpdateFrequencyCount.GetValueOrDefault())
        {
          ...
          local.ProgramCheckpointRestart.RestartInfo =
            ...
            "3-16    " + " " + local.BatchTimestampWorkArea.TextTimestamp;
          local.ProgramCheckpointRestart.RestartInfo =
...
ReadEach:
  ...
  private void CreateDashboardStagingPriority35()
  {
    ...
    var daysToReturnOfSrvcNumer =
      local.Local1.Item.G.DaysToReturnOfSrvcNumer.GetValueOrDefault();
    var daysToReturnOfServiceDenom =
      local.Local1.Item.G.DaysToReturnOfServiceDenom.GetValueOrDefault();
    var daysToReturnOfServiceAvg =
      local.Local1.Item.G.DaysToReturnOfServiceAvg.GetValueOrDefault();
  ...
  private IEnumerable<bool> ReadLegalActionServiceProcess()
  {
    entities.ServiceProcess.Populated = false;
    entities.LegalAction.Populated = false;

    return ReadEach("ReadLegalActionServiceProcess",
      (db, command) =>
      {
    ...
    return ReadEach("ReadLegalReferralCase",
      (db, command) =>
      {
        db.SetDate(
          ...
          entities.ServiceProcess.ReturnDate.GetValueOrDefault());
        db.SetNullableDate(
          command, "statusDate",
