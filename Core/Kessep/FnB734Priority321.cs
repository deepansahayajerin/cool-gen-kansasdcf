// Program: FN_B734_PRIORITY_3_21, ID: 945148938, model: 746.
// Short name: SWE03695
using System;
...
/// <summary>
/// <para>
/// A program: FN_B734_PRIORITY_3_21.
/// </para>
/// <para>
...
/// </summary>
[Serializable]
public partial class FnB734Priority321: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_B734_PRIORITY_3_21 program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnB734Priority321(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnB734Priority321.
  /// </summary>
  public FnB734Priority321(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // Priority 3-21: Referrals to Legal for Establishment
    // -------------------------------------------------------------------------------------
    // This is a count of all establishment referrals to legal completed by a 
    ...
    // 2)	Count only referrals with one or both of the following Referral Reason
    // codes: EST, PAT.  Do not count referral if any other Referral Reason
    // code exists.
    // 3)	Exclude referrals in R(ejected) or W(ithdrawn) status as of refresh 
    // date (Do not count if the referral was created in the report period, but
    // was subsequently withdrawn or rejected in the same report period).
    ...
    //  089-089    Blank
    //  090-110    Legal Refferal createtimestamp
    ...
    {
      if (Equal(import.ProgramCheckpointRestart.RestartInfo, 81, 4, "3-21"))
      {
        ...
        else
        {
          ...
          foreach(var item in ReadDashboardStagingPriority2())
          {
            ...
            local.Local1.Update.G.Assign(entities.DashboardStagingPriority35);
            local.Local1.Update.G.ReferralsToLegalForEst = 0;
          }
    ...
    foreach(var item in ReadLegalReferralCase())
    {
        ...
        (entities.LegalReferral.ReferralReason4, "EST"))
      {
        ...
        if (Lt(entities.CaseAssignment.DiscontinueDate,
          import.ReportEndDate.Date) && Lt
          (local.Null1.Date, entities.CaseAssignment.DiscontinueDate))
        {
          local.CaseAssginemnt.Date = entities.CaseAssignment.DiscontinueDate;
        }
        else
        {
          local.CaseAssginemnt.Date = import.ReportEndDate.Date;
        }
        ...
        local.DashboardAuditData.DashboardPriority = "3-21";
        ...
        if (!IsEmpty(local.DashboardAuditData.JudicialDistrict))
        {
          ...
          local.Local1.Update.G.ReferralsToLegalForEst =
            local.Local1.Item.G.ReferralsToLegalForEst.GetValueOrDefault() + 1;
        }
      }
      else
      {
    ...
    local.ProgramCheckpointRestart.RestartInfo = "";
    local.ProgramCheckpointRestart.RestartInfo =
      Substring(import.ProgramCheckpointRestart.RestartInfo, 250, 1, 80) + "3-22    ";
  ...
  private void UseFnB734DetermineJdFromCase()
  {
    ...
    useImport.ReportEndDate.Date = local.CaseAssginemnt.Date;
  ...
  private void CreateDashboardStagingPriority35()
  {
    ...
    var referralsToLegalForEst =
      local.Local1.Item.G.ReferralsToLegalForEst.GetValueOrDefault();
  ...
  private IEnumerable<bool> ReadLegalReferralCase()
  {
    ...
    return ReadEach("ReadLegalReferralCase",
      ...
      (db, reader) =>
      {
        ...
        entities.Case1.NoJurisdictionCd = db.GetNullableString(reader, 10);
...
#region Structures
  ...
  public class Local
  {
    ...
    /// <summary>
    /// A value of CaseAssginemnt.
    /// </summary>
    [JsonPropertyName("caseAssginemnt")]
    public DateWorkArea CaseAssginemnt
    {
      get => caseAssginemnt ??= new();
      set => caseAssginemnt = value;
    }
    ...
    private DateWorkArea caseAssginemnt;
