// Program: FN_B791_IVR_EXTRACT, ID: 1902413951, model: 746.
// Short name: SWEB791P
using System;
...
/// <summary>
/// A program: FN_B791_IVR_EXTRACT.
/// </summary>
[Serializable]
[ProcedureStep(ProcedureType.Batch)]
public partial class FnB791IvrExtract: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_B791_IVR_EXTRACT program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnB791IvrExtract(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnB791IvrExtract.
  /// </summary>
  public FnB791IvrExtract(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // --------  ----------	----------	
    // ---------------------------------------------------------------------------------
    // 10/01/2013 DDupree	CQ38344 	Initial Development.  This is a copy of the 
    // AR statements (SWEFB663).
    // -------------------------------------------------------------------------------------------------------------------------
    // -------------------------------------------------------------------------------------------------------------------------
    // --  This program extracts collection and payment request information 
    // needed to produce a file to send to the call center with AR statements
    // info.
    // --
    ...
    // --  provided as input to SWEFB792 which actually creates the file sent to
    // the call center.
    // --
    ...
    if (AsChar(local.Common.Flag) == 'Y')
    {
      ...
      do
      {
        if (!IsEmpty(local.ReportingAr.Number))
        {
          ...
          local.Obligor.Number = local.Group.Item.GlocalObligor.Number;
        }
        ...
        foreach(var item in ReadCollectionCashReceiptDetailCashReceiptEvent())
        {
          ...
          if (!Equal(entities.Obligor1.Number, local.PreviousObligor.Number))
          {
            ...
            local.CsePersonsWorkSet.Number = entities.Obligor1.Number;
          ...
          if (!IsExitState("ACO_NN0000_ALL_OK"))
          {
            ...
            {
              local.EabReportSend.RptDetail = "AR=" + local.Ar.Number;
              local.EabReportSend.RptDetail =
          ...
          if (!IsEmpty(local.ReportingAr.Number) && !
            Equal(local.ReportingAr.Number, local.Ar.Number))
          {
          ...
          UseFnB663WriteExtractDataFile1();
      ...
      // -------------------------------------------------------------------------------------------------------------------------
      for(local.Counter.Count = 1; local.Counter.Count <= 8; ++
        local.Counter.Count)
      {
        switch(local.Counter.Count)
        {
          ...
          case 2:
            local.EabReportSend.RptDetail =
              "Collections Not Extracted Due To Errors  " + NumberToString
              (local.NumberOfErrors.Count, 10, 6);
          ...
          case 4:
            local.EabReportSend.RptDetail =
              "Number of TAF Collections Extracted      " + NumberToString
              (local.NumberOfTafCollections.Count, 10, 6);

            break;
          case 5:
            local.EabReportSend.RptDetail =
              "Number of NA Collections Extracted       " + NumberToString
              (local.NumberOfNaCollections.Count, 10, 6);
          ...
          case 7:
            local.EabReportSend.RptDetail =
              "Total Number of Collections Extracted    " + NumberToString
              ((long)local.NumberOfTafCollections.Count +
              local.NumberOfNaCollections.Count, 10, 6);
...
Test:
    ...
    UseFnB663WriteExtractDataFile2();
  ...
  private void UseFnB663DetermineAr()
  {
    ...
    MoveCsePerson(useExport.Ar, local.Ar);
  }

  private void UseFnB663WriteExtractDataFile1()
  {
  ...
  private void UseFnB663WriteExtractDataFile2()
  {
    var useImport = new FnB663WriteExtractDataFile.Import();
    var useExport = new FnB663WriteExtractDataFile.Export();
  ...
  private void UseFnB791BatchInitialization()
  {
    var useImport = new FnB791BatchInitialization.Import();
    var useExport = new FnB791BatchInitialization.Export();

    Call(FnB791BatchInitialization.Execute, useImport, useExport);
  ...
  private IEnumerable<bool> ReadCollectionCashReceiptDetailCashReceiptEvent()
  {
    ...
    return ReadEach("ReadCollectionCashReceiptDetailCashReceiptEvent",
      (db, command) =>
      {
        ...
        db.SetString(command, "number", local.Obligor.Number);
      ...
      (db, reader) =>
      {
        ...
        entities.Collection.ArNumber = db.GetNullableString(reader, 19);
        entities.CashReceiptDetail.CltIdentifier =
          db.GetNullableInt32(reader, 20);
        entities.Debt.CspSupNumber = db.GetNullableString(reader, 21);
        entities.Debt.CpaSupType = db.GetNullableString(reader, 22);
