// Program: FN_BF18_REVERSE_MED_JUDGMENT_RET, ID: 371360978, model: 746.
// Short name: SWEFF18B
using System;
...
/// <summary>
/// A program: FN_BF18_REVERSE_MED_JUDGMENT_RET.
/// </summary>
[Serializable]
[ProcedureStep(ProcedureType.Batch)]
public partial class FnBf18ReverseMedJudgmentRet: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_BF18_REVERSE_MED_JUDGMENT_RET program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnBf18ReverseMedJudgmentRet(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnBf18ReverseMedJudgmentRet.
  /// </summary>
  public FnBf18ReverseMedJudgmentRet(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // This program is to reverse the adjustments made by the SWEFBF17 program 
    // for the date specified in the parameter.
    // 01-08-2009  SWCOAMX - Arun      CQ#8678 Added Obligation Id, Current bal 
    // due and Debt Status History Created by to the output file.
    ...
    if (IsExitState("ACO_NN0000_ALL_OK"))
    {
      if (AsChar(local.ProgramCheckpointRestart.RestartInd) == 'Y')
      {
        ...
        local.TotalDebtAdjProcessed.Count =
          (int)StringToNumber(Substring(
    ...
    foreach(var item in ReadObligationDebtAdjustment())
    {
      // : Read Debt adjustments that need to be reversed.
      local.ActivityByWorker.Flag = "N";

      // *** CQ#8678 Changes Start Here ***
      local.FileObligationId.SystemGeneratedIdentifier =
        entities.Obligation.SystemGeneratedIdentifier;
      local.FileCreatedBy.CreatedBy = "";

      // *** CQ#8678 Changes End   Here ***
      if (ReadDebtDetailDebt())
      {
        // : Debt detail found for the adjustment to be reversed
        ++local.TotalDebtDtlProcessed.Count;

        // *** CQ#8678 Commented the below code ***
        // *** CQ#8678 Comment Starts Here ***
        // *** CQ#8678 Comment Ends  Here ***
        // *** CQ#8678 Changes Starts Here ***
        if (ReadDebtDetailStatusHistory2())
        {
          if (Equal(entities.DebtDetailStatusHistory.CreatedBy, "SWEFBF17") && Equal
            (entities.DebtDetailStatusHistory.CreatedTmst,
            local.ParameterProcess.Timestamp))
          {
            // No Debt Adjustment done after the MJ write-off
          }
          else
          {
            local.FileCreatedBy.CreatedBy =
              entities.DebtDetailStatusHistory.CreatedBy;
            local.ActivityByWorker.Flag = "Y";
          }
        }

        if (AsChar(local.ActivityByWorker.Flag) == 'N')
        {
      ...
      local.FileBalDueAtRetiredDt.BalanceDueAmt = entities.ToBeReversed.Amount;
      ...
      // *** CQ#8678 Changes Start Here ***
      local.FileCurrentBalDue.BalanceDueAmt = entities.DebtDetail.BalanceDueAmt;

      // *** CQ#8678 Changes End   Here ***
      if (AsChar(local.RunType.Flag) == 'U' && AsChar
        (local.ActivityByWorker.Flag) == 'N')
      {
        // : Create Debt Adjustment only in the Update mode and if there was no 
        // activity by the worker after the write-off was done.
...
Test1:
        ...
        local.DebtDetailStatusHistory.ReasonTxt =
          "DA is balance owed at time of statewide MJ reactivation.";

        if (ReadDebtDetailStatusHistory1())
        {
          try
          {
...
Test2:
      ...
      // Read Obligor that owes the obligation.
      if (ReadCsePerson2())
      {
      ...
      // Read the Supported person on the debt
      if (ReadCsePerson1())
      {
      ...
      if (AsChar(local.ActivityByWorker.Flag) == 'Y')
      {
        local.FileRemarks.Text30 = "Skipped due to worker activity";
      }
      else
      {
        ++local.TotalDebtAdjProcessed.Count;
      ...
      // *** CQ#8678 Changes Start Here ***
      local.FileTypeOfProcess.Text1 = "R";
        ...
        .ProgramCheckpointRestart.UpdateFrequencyCount.GetValueOrDefault())
      {
        ...
        local.ProgramCheckpointRestart.RestartInfo =
          ...
          (local.TotalDebtAdjProcessed.Count, 9, 7) + " " + NumberToString
...
ReadEach:
    ...
    if (!IsExitState("ACO_NN0000_ALL_OK"))
    {
      if (IsExitState("FN0000_DEBT_DETAIL_NF_RB"))
      {
        local.NeededToWrite.RptDetail =
          "Debt detail not found for obligation : " + NumberToString
          (entities.Obligation.SystemGeneratedIdentifier, 7, 9);
      }
      else if (IsExitState("FN0000_OBLIGOR_NF_RB"))
      {
        local.NeededToWrite.RptDetail =
          "Obligor not found for obligation : " + NumberToString
          (entities.Obligation.SystemGeneratedIdentifier, 7, 9);
      }
      else if (IsExitState("SUPPORTED_PERSON_NF_RB"))
      {
        local.NeededToWrite.RptDetail =
          "Supported person not found for obligation : " + NumberToString
          (entities.Obligation.SystemGeneratedIdentifier, 7, 9);
      }
    ...
    {
      local.NeededToWrite.RptDetail =
        "Total Debt Debt Adjustments Processed and Updated :";
    }
    else
    {
      local.NeededToWrite.RptDetail =
        "Total Debt Debt Adjustments Processed             :";
    }

    local.NeededToWrite.RptDetail = TrimEnd(local.NeededToWrite.RptDetail) + " " +
      NumberToString(local.TotalDebtAdjProcessed.Count, 9, 7);
    UseCabControlReport3();
  ...
  private void UseFnEabSwexfw18RetireMjWrite1()
  {
    ...
    useImport.ObligationId.SystemGeneratedIdentifier =
      local.FileObligationId.SystemGeneratedIdentifier;
    useImport.CurrentBalDue.BalanceDueAmt =
      local.FileCurrentBalDue.BalanceDueAmt;
    useImport.CreatedBy.CreatedBy = local.FileCreatedBy.CreatedBy;
  ...
  private void CreateDebtAdjustment1()
  {
    ...
    var type1 = "DA";
    var amount = entities.ToBeReversed.Amount;
    var debtAdjustmentType = "I";
  ...
  {
    System.Diagnostics.Debug.Assert(entities.ToBeReversed.Populated);
    ...
    return Read("ReadDebtDetailDebt",
      (db, command) =>
      {
        db.SetInt32(command, "otyTypeSecondary", entities.ToBeReversed.OtyType);
        db.SetString(command, "otrType", entities.ToBeReversed.Type1);
        db.SetInt32(
          command, "otrGeneratedId",
          entities.ToBeReversed.SystemGeneratedIdentifier);
        db.SetString(command, "cpaType", entities.ToBeReversed.CpaType);
        db.SetString(command, "cspNumber", entities.ToBeReversed.CspNumber);
        db.SetInt32(
          command, "obgGeneratedId", entities.ToBeReversed.ObgGeneratedId);
      },
      (db, reader) =>
      {
  ...
  private bool ReadDebtDetailStatusHistory1()
  {
    System.Diagnostics.Debug.Assert(entities.DebtDetail.Populated);
    entities.DebtDetailStatusHistory.Populated = false;

    return Read("ReadDebtDetailStatusHistory1",
      (db, command) =>
      {
  ...
  private bool ReadDebtDetailStatusHistory2()
  {
    System.Diagnostics.Debug.Assert(entities.DebtDetail.Populated);
    entities.DebtDetailStatusHistory.Populated = false;

    return Read("ReadDebtDetailStatusHistory2",
      (db, command) =>
      {
        db.SetInt32(command, "otyType", entities.DebtDetail.OtyType);
        db.SetInt32(command, "obgId", entities.DebtDetail.ObgGeneratedId);
  ...
  private IEnumerable<bool> ReadObligationDebtAdjustment()
  {
    entities.ToBeReversed.Populated = false;
    entities.Obligation.Populated = false;

    return ReadEach("ReadObligationDebtAdjustment",
      (db, command) =>
      {
        db.SetDateTime(
      ...
      (db, reader) =>
      {
        ...
        entities.ToBeReversed.CpaType = db.GetString(reader, 0);
        entities.Obligation.CspNumber = db.GetString(reader, 1);
        entities.ToBeReversed.CspNumber = db.GetString(reader, 1);
        entities.Obligation.SystemGeneratedIdentifier = db.GetInt32(reader, 2);
        entities.ToBeReversed.ObgGeneratedId = db.GetInt32(reader, 2);
        entities.Obligation.DtyGeneratedId = db.GetInt32(reader, 3);
        entities.ToBeReversed.OtyType = db.GetInt32(reader, 3);
        ...
        entities.ToBeReversed.Type1 = db.GetString(reader, 8);
        entities.ToBeReversed.Amount = db.GetDecimal(reader, 9);
        entities.ToBeReversed.DebtAdjustmentType = db.GetString(reader, 10);
        entities.ToBeReversed.DebtAdjustmentDt = db.GetDate(reader, 11);
        entities.ToBeReversed.CreatedBy = db.GetString(reader, 12);
        entities.ToBeReversed.CreatedTmst = db.GetDateTime(reader, 13);
        entities.ToBeReversed.CspSupNumber = db.GetNullableString(reader, 14);
        entities.ToBeReversed.CpaSupType = db.GetNullableString(reader, 15);
        entities.ToBeReversed.DebtAdjustmentProcessDate =
          db.GetDate(reader, 16);
        entities.ToBeReversed.DebtAdjCollAdjProcReqInd =
          db.GetString(reader, 17);
        entities.ToBeReversed.ReverseCollectionsInd =
          db.GetNullableString(reader, 18);
        entities.ToBeReversed.Populated = true;
        entities.Obligation.Populated = true;
        CheckValid<Obligation>("CpaType", entities.Obligation.CpaType);
        CheckValid<ObligationTransaction>("CpaType",
          entities.ToBeReversed.CpaType);
        CheckValid<Obligation>("OrderTypeCode",
          entities.Obligation.OrderTypeCode);
        CheckValid<ObligationTransaction>("Type1", entities.ToBeReversed.Type1);
        CheckValid<ObligationTransaction>("DebtAdjustmentType",
          entities.ToBeReversed.DebtAdjustmentType);
        CheckValid<ObligationTransaction>("CpaSupType",
          entities.ToBeReversed.CpaSupType);
        CheckValid<ObligationTransaction>("DebtAdjCollAdjProcReqInd",
          entities.ToBeReversed.DebtAdjCollAdjProcReqInd);
  ...
  private void UpdateDebtDetail()
  {
    ...
    var balanceDueAmt = entities.ToBeReversed.Amount;
    var retiredDt = local.Null1.Date;
...
#region Structures
  ...
  {
    /// <summary>
    /// A value of ActivityByWorker.
    /// </summary>
    [JsonPropertyName("activityByWorker")]
    public Common ActivityByWorker
    {
      get => activityByWorker ??= new();
      set => activityByWorker = value;
    }
    ...
    /// <summary>
    /// A value of TotalDebtAdjProcessed.
    /// </summary>
    [JsonPropertyName("totalDebtAdjProcessed")]
    public Common TotalDebtAdjProcessed
    {
      get => totalDebtAdjProcessed ??= new();
      set => totalDebtAdjProcessed = value;
    }
    ...
    /// <summary>
    /// A value of FileCurrentBalDue.
    /// </summary>
    [JsonPropertyName("fileCurrentBalDue")]
    public DebtDetail FileCurrentBalDue
    {
      get => fileCurrentBalDue ??= new();
      set => fileCurrentBalDue = value;
    }
    ...
    {
      activityByWorker = null;
      totalDebtDtlProcessed = null;
      totalDebtAdjProcessed = null;
      childActive = null;
      apActive = null;
      caseOpen = null;
      fileRemarks = null;
      ...
      fileCurrentBalDue = null;
      fileCreatedBy = null;
    }

    private Common activityByWorker;
    private Common totalDebtDtlProcessed;
    private Common totalDebtAdjProcessed;
    ...
    private DebtDetail fileCurrentBalDue;
  ...
  {
    /// <summary>
    /// A value of ToBeReversed.
    /// </summary>
    [JsonPropertyName("toBeReversed")]
    public ObligationTransaction ToBeReversed
    {
      get => toBeReversed ??= new();
      set => toBeReversed = value;
    }
    ...
    private ObligationTransaction toBeReversed;
