// Program: FN_BFXE_CONVERT_POSITIVE_URA_ADJ, ID: 374447942, model: 746.
// Short name: SWEFFXEB
using System;
...
/// <summary>
/// A program: FN_BFXE_CONVERT_POSITIVE_URA_ADJ.
/// </summary>
[Serializable]
[ProcedureStep(ProcedureType.Batch)]
public partial class FnBfxeConvertPositiveUraAdj: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_BFXE_CONVERT_POSITIVE_URA_ADJ program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnBfxeConvertPositiveUraAdj(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnBfxeConvertPositiveUraAdj.
  /// </summary>
  public FnBfxeConvertPositiveUraAdj(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  {
    // -----------------------------------------------------------
    // Initial Version :- SWSRKXD 04/18/2000
    // This utility will load im_hh_mbr_mnth_adj from ura_adjustments.
    // -----------------------------------------------------------
    // -----------------------------------------------------
    // 4/25/2000
    // Following a meeting with SMEs Ann M. and Kim W,  it has
    // been agreed upon that we will only convert household_ura
    // adjustments (both pre and post conversion records)
    // The remaining 5 types of ajustments will need to be entered
    // manually using the new URAA screen.
    // -----------------------------------------------------
    // -----------------------------------------------------------
    // 06/19/2000
    // If no Grant is posted for Adjustment Year/Month, go back in
    // time to get the most recent Grant and distribute Adj amount
    // equally amongst members which received Grants during that
    // month. If there was no Grant posted 'ever', error off this
    // Adjustment record.
    // -----------------------------------------------------------
    // -----------------------------------------------------------
    // 08/05/2000
    // Significant changes to prorate Adj amounts and apply to member's URA.
    // -----------------------------------------------------------
    // -----------------------------------------------------------
    // 08/15/2000
    // Split this conversion into 2 seperate processes. This process
    // will handle positive URA Adjustments. Negative adjustments
    // will be converted by BFXG.
    // -----------------------------------------------------------
    ExitState = "ACO_NN0000_ALL_OK";
    local.ProgramProcessingInfo.Name = "SWEFBFXE";
    ...
    if (AsChar(local.ProgramCheckpointRestart.RestartInd) == 'Y')
    {
      ...
      local.EabReportSend.RptDetail =
        "Program SWEFBFXE will be restarted. AE Case #:" + local
    ...
    foreach(var item in ReadUraAdjustmentImHouseholdMemberImHousehold())
    {
      ...
      if (IsEmpty(local.DummyLoop.Flag))
      {
        ...
        // -------------------------------------------------------------
        // Build a GV of all members and corresponding URA amounts.
        // ------------------------------------------------------------
        ...
        foreach(var item1 in ReadCsePerson())
        {
          ...
          // ------------------------------------------------------------
          // Calculate total 'member' URA till Adj month.
          // ------------------------------------------------------------
