// Program: FN_BFXF_BATCH_INITIALIZATION, ID: 374485941, model: 746.
// Short name: SWE02876
using System;
...
/// <summary>
/// A program: FN_BFXF_BATCH_INITIALIZATION.
/// </summary>
[Serializable]
public partial class FnBfxfBatchInitialization: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_BFXF_BATCH_INITIALIZATION program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnBfxfBatchInitialization(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnBfxfBatchInitialization.
  /// </summary>
  public FnBfxfBatchInitialization(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    if (IsExitState("ACO_NN0000_ALL_OK"))
    {
      ...
      if (!IsEmpty(Substring(export.ProgramProcessingInfo.ParameterList, 3, 8)))
      {
        export.TestCollection.CollectionDt =
          IntToDate((int)StringToNumber(Substring(
            export.ProgramProcessingInfo.ParameterList, 3, 8)));
      }

      if (!IsEmpty(Substring(export.ProgramProcessingInfo.ParameterList, 11, 10)))
        
      {
        export.TestCashReceiptDetail.ObligorPersonNumber =
          Substring(export.ProgramProcessingInfo.ParameterList, 11, 10);
      }
    }
    else
    {
    ...
    if (export.TestCollection.CollectionDt != null)
    {
      local.EabReportSend.RptDetail =
        "This run will only process Collection Date: " + Substring
        (export.ProgramProcessingInfo.ParameterList, 3, 8);
      UseCabControlReport1();
    ...
    if (!IsEmpty(export.TestCashReceiptDetail.ObligorPersonNumber))
    {
      local.EabReportSend.RptDetail =
        "This run will only process Obligor #: " + Substring
        (export.ProgramProcessingInfo.ParameterList, 11, 10);
      UseCabControlReport1();
    ...
    if (AsChar(export.TestRunInd.Flag) == 'Y' || AsChar
      (export.DisplayInd.Flag) == 'Y' || export.TestCollection.CollectionDt != null
      || !IsEmpty(export.TestCashReceiptDetail.ObligorPersonNumber))
    {
      local.EabReportSend.RptDetail = "";
      UseCabControlReport1();
