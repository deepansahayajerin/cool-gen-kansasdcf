// Program: FN_BUILD_HOUSEHOLD_HISTORY_TABLE, ID: 374486313, model: 746.
// Short name: SWE02399
using System;
...
/// <summary>
/// A program: FN_BUILD_HOUSEHOLD_HISTORY_TABLE.
/// </summary>
[Serializable]
public partial class FnBuildHouseholdHistoryTable: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_BUILD_HOUSEHOLD_HISTORY_TABLE program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnBuildHouseholdHistoryTable(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnBuildHouseholdHistoryTable.
  /// </summary>
  public FnBuildHouseholdHistoryTable(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    foreach(var item in ReadCsePersonObligation())
    {
      ...
      if (export.Legal.Index + 1 == Export.LegalGroup.Capacity)
      {
        break;
      }
...
Test:
      ...
      export.Legal.Index)
    {
      ...
      foreach(var item in ReadLegalAction2())
      {
        ...
        {
          ++export.Legal.Item.LegalDtl.Index;
          export.Legal.Item.LegalDtl.CheckSize();
        }
...
Next1:
    ...
    // : Build a temporary table of all Members involved in the household's that
    // the original Supported Person or Legal Action Person's are related too.
    for(export.Legal.Index = 0; export.Legal.Index < export.Legal.Count; ++
      export.Legal.Index)
    {
      if (!export.Legal.CheckSize())
...
AfterCycle2:
      ...
      local.SuppPrsn.Index)
    {
      ...
      {
        if (entities.ExistingImHouseholdMbrMnthlySum.Year == local
          .Collection.Year && entities.ExistingImHouseholdMbrMnthlySum.Month > local
          .Collection.Month)
        {
          continue;
        }
...
#region Structures
  ...
  public class Entities
  {
    ...
    public LegalActionDetail Exsting
    {
      get => exsting ??= new();
      set => exsting = value;
    }
    ...
    private LegalActionDetail exsting;
  }
#endregion
}
