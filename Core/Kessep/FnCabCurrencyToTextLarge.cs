// Program: FN_CAB_CURRENCY_TO_TEXT_LARGE, ID: 1902606946, model: 746.
// Short name: SWE01239
using System;
...
/// <summary>
/// <para>
/// A program: FN_CAB_CURRENCY_TO_TEXT_LARGE.
/// </para>
/// <para>
...
/// </summary>
[Serializable]
public partial class FnCabCurrencyToTextLarge: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_CAB_CURRENCY_TO_TEXT_LARGE program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnCabCurrencyToTextLarge(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnCabCurrencyToTextLarge.
  /// </summary>
  public FnCabCurrencyToTextLarge(IContext context, Import import, Export export)
    :
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    if (local.Common.TotalCurrency == 0)
    {
      if (AsChar(import.ReturnAnInteger.Flag) == 'Y')
      {
        local.FormattedTextField.Text20 = "                   0";
        local.SizeOfFormattedNumber.Count = 1;
      }
      else
      {
        local.FormattedTextField.Text20 = "               $0.00";
    ...
    else
    {
      ...
      if (AsChar(import.CommasRequired.Flag) == 'Y')
      {
        ...
        // : There are only 3 possible commas.
        ...
        local.WorkSeg3.Text3 = Substring(local.FormattedTextField.Text15, 7, 3);
        local.Large.Text1 = Substring(local.FormattedTextField.Text15, 6, 1);
        local.TextWorkArea.Text1 =
          Substring(local.FormattedTextField.Text15, 9, 1);

        if (IsEmpty(local.Large.Text1) || AsChar(local.Large.Text1) == AsChar
          (local.Sign.Text1))
        {
            ...
            (local.TextWorkArea.Text1) == AsChar(local.Sign.Text1))
          {
              ...
              (local.TextWorkArea.Text1) == AsChar(local.Sign.Text1))
            {
              local.FormattedTextField.Text15 =
                ...
                WorkArea.Text15_MaxLength, 1, 12) + local.WorkSeg2.Text3;
            }
            else
            {
              local.FormattedTextField.Text15 =
                Substring(local.FormattedTextField.Text15,
                WorkArea.Text15_MaxLength, 1, 8) + local.WorkSeg1.Text3 + ","
                + local.WorkSeg2.Text3;
        ...
        else if (IsEmpty(local.TextWorkArea.Text1) || AsChar
          (local.TextWorkArea.Text1) == AsChar(local.Sign.Text1))
        {
          local.TextWorkArea.Text1 =
            Substring(local.FormattedTextField.Text15, 12, 1);
            ...
            (local.TextWorkArea.Text1) == AsChar(local.Sign.Text1))
          {
            local.FormattedTextField.Text15 =
              Substring(local.FormattedTextField.Text15,
              WorkArea.Text15_MaxLength, 1, 11) + local.WorkSeg2.Text3;
          }
          else
          {
            local.FormattedTextField.Text15 =
        ...
        else
        {
          local.FormattedTextField.Text15 =
            Substring(local.FormattedTextField.Text15,
            WorkArea.Text15_MaxLength, 4, 3) + "," + local.WorkSeg3.Text3 + ","
            + local.WorkSeg1.Text3 + "," + local.WorkSeg2.Text3;
          local.SizeOfFormattedNumber.Count += 3;
        }
      }
      ...
      {
        local.Verify.Count = Verify(local.FormattedTextField.Text15, " ");

        if (local.Verify.Count >= 4)
        {
          local.FormattedTextField.Text15 =
            Substring(local.FormattedTextField.Text15,
            WorkArea.Text15_MaxLength, local.Verify.Count, 16 -
            local.Verify.Count) + "." + local.TextWorkArea.Text2;

          if (local.SizeOfFormattedNumber.Count <= 13)
          {
            local.FormattedTextField.Text20 =
              Substring(local.Spaces.Text20, WorkArea.Text20_MaxLength, 1, 20 -
              (local.SizeOfFormattedNumber.Count + 1)) + "$" + Substring
              (local.FormattedTextField.Text15, WorkArea.Text15_MaxLength, 1,
              local.SizeOfFormattedNumber.Count + 2);
          }
          else if (local.SizeOfFormattedNumber.Count == 14)
          {
            local.FormattedTextField.Text20 = "     $" + Substring
              (local.FormattedTextField.Text15, WorkArea.Text15_MaxLength, 1, 14);
              
          }
          else
          {
            local.FormattedTextField.Text20 = "    $" + local
              .FormattedTextField.Text15;
          }
        }
        else
        {
          local.FormattedTextField.Text20 =
            Substring(local.Spaces.Text20, WorkArea.Text20_MaxLength, 1, 5 -
            (local.SizeOfFormattedNumber.Count - 14)) + "$" + Substring
            (local.FormattedTextField.Text15, WorkArea.Text15_MaxLength,
            local.Verify.Count, 15 - local.Verify.Count + 1) + "." + local
            .TextWorkArea.Text2;
        }
      }
    }
    ...
    export.WorkArea.Text20 = local.FormattedTextField.Text20;
  }
...
#region Structures
  ...
  public class Local
  {
    ...
    /// A value of WorkSeg4.
    /// </summary>
    [JsonPropertyName("workSeg4")]
    public WorkArea WorkSeg4
    {
      get => workSeg4 ??= new();
      set => workSeg4 = value;
    }
    ...
    /// A value of WorkSeg3.
    /// </summary>
    [JsonPropertyName("workSeg3")]
    public WorkArea WorkSeg3
    {
      get => workSeg3 ??= new();
      set => workSeg3 = value;
    }
    ...
    private TextWorkArea large;
    private WorkArea workSeg4;
    private WorkArea workSeg3;
    private Common common;
