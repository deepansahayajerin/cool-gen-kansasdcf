// Program: FN_CAB_NLIST_CASH_RECEIPT, ID: 372461945, model: 746.
// Short name: SWE02268
using System;
...
/// <summary>
/// <para>
/// A program: FN_CAB_NLIST_CASH_RECEIPT.
/// </para>
/// <para>
/// RESP: FINANCE
...
/// </summary>
[Serializable]
public partial class FnCabNlistCashReceipt: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_CAB_NLIST_CASH_RECEIPT program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnCabNlistCashReceipt(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnCabNlistCashReceipt.
  /// </summary>
  public FnCabNlistCashReceipt(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // - - ADD Starting Check Number
    // 09/23/99  J Katz  H00074126 [continued]
    // - - RECODE Check Number to work on exact match.
    // * * * * * * * * * * * * * * * * * * * * * * *
    local.Max.Date = UseCabSetMaximumDiscontinueDate();

    if (!IsEmpty(import.FilterCashReceiptStatus.Code))
    {
    ...
    if (!IsEmpty(import.FilterCashReceiptSourceType.Code))
    {
    ...
    if (IsEmpty(import.FilterCashReceiptSourceType.Code))
    {
      if (IsEmpty(import.FilterCashReceipt.CreatedBy))
      {
        if (IsEmpty(import.FilterCashReceiptStatus.Code))
        {
          // ******
          ...
          foreach(var item in ReadCashReceiptCashReceiptEventCashReceiptType4())
          {
            ...
            foreach(var item1 in ReadCashReceiptDetailStatus())
            {
              ...
              if (Equal(entities.CashReceiptDetailStatus.Code, "REC"))
              {
                local.Detail.Flag = "N";
    ...
    else
    {
      ...
      else if (IsEmpty(import.FilterCashReceiptStatus.Code))
      {
        // ******
        // cash receipt source type code and
  ...
  private IEnumerable<bool> ReadCashReceiptCashReceiptEventCashReceiptType1()
  {
    return ReadEach("ReadCashReceiptCashReceiptEventCashReceiptType1",
      (db, command) =>
      {
        db.SetString(command, "createdBy", import.FilterCashReceipt.CreatedBy);
        db.SetInt32(
        ...
        db.SetNullableString(
          command, "checkType", import.FilterCashReceipt.CheckType ?? "");
        db.SetNullableString(
          command, "checkNumber", import.Starting.CheckNumber ?? "");
  ...
  private bool ReadCashReceiptSourceType1()
  {
    ...
    return Read("ReadCashReceiptSourceType1",
      (db, command) =>
      {
        db.SetString(command, "code", import.FilterCashReceiptSourceType.Code);
      },
      (db, reader) =>
      {
        entities.ExistingCashReceiptSourceType.SystemGeneratedIdentifier =
  ...
  private bool ReadCashReceiptStatus()
  {
    ...
    return Read("ReadCashReceiptStatus",
      (db, command) =>
      {
        db.SetString(command, "code", import.FilterCashReceiptStatus.Code);
      },
      (db, reader) =>
      {
...
#region Structures
  ...
  public class Import
  {
    ...
    /// <summary>
    /// A value of FilterCashReceipt.
    /// </summary>
    [JsonPropertyName("filterCashReceipt")]
    public CashReceipt FilterCashReceipt
    {
      get => filterCashReceipt ??= new();
      set => filterCashReceipt = value;
    }

    /// <summary>
    /// A value of FilterCashReceiptStatus.
    /// </summary>
    [JsonPropertyName("filterCashReceiptStatus")]
    public CashReceiptStatus FilterCashReceiptStatus
    {
      get => filterCashReceiptStatus ??= new();
      set => filterCashReceiptStatus = value;
    }

    /// <summary>
    /// A value of FilterCashReceiptSourceType.
    /// </summary>
    [JsonPropertyName("filterCashReceiptSourceType")]
    public CashReceiptSourceType FilterCashReceiptSourceType
    {
      get => filterCashReceiptSourceType ??= new();
      set => filterCashReceiptSourceType = value;
    }
    ...
    private CashReceipt filterCashReceipt;
    private CashReceiptStatus filterCashReceiptStatus;
    private CashReceiptSourceType filterCashReceiptSourceType;
