// Program: FN_COMPUTE_TOTALS_FOR_CT_ORDER, ID: 370994658, model: 746.
// Short name: SWE01753
using System;
...
/// <summary>
/// A program: FN_COMPUTE_TOTALS_FOR_CT_ORDER.
/// </summary>
[Serializable]
public partial class FnComputeTotalsForCtOrder: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_COMPUTE_TOTALS_FOR_CT_ORDER program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnComputeTotalsForCtOrder(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnComputeTotalsForCtOrder.
  /// </summary>
  public FnComputeTotalsForCtOrder(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  {
    // -------------------------------------------------------------------
    // Initial Code       Madhu Kumar
    // -------------------------------------------------------------------
    // PR# 75972      VITHAL MADHIRA
    ...
    foreach(var item in ReadObligationTypeObligationDebtDebtDetail())
    {
      if (AsChar(entities.ExistingObligation.PrimarySecondaryCode) == 'J')
      {
        ...
        {
          // *********************************************************
          //   This means that he is the first person in the joint
          // several and hence continue with the computations.
          // *********************************************************
        }
      }
      ...
      if (AsChar(entities.ExistingObligationType.Classification) == 'A')
      {
        ...
        else
        {
            ...
            local.HardcodedMsType.SystemGeneratedIdentifier)
          {
            ...
            local.HardcodedMcType.SystemGeneratedIdentifier)
          {
            export.ScreenOwedAmountsDtl.McCurrArrears += entities.
              ExistingDebtDetail.BalanceDueAmt;
          }
    ...
    if (AsChar(import.OmitCollectionDtlsInd.Flag) != 'Y')
    {
      ...
      // --- Vithal
      // ---------------------------------------------------------------
      foreach(var item in ReadCollectionObligationTypeObligationObligor())
      {
        if (AsChar(local.OmitSecondaryObligInd.Flag) == 'Y')
        {
          if (AsChar(entities.ExistingObligation.PrimarySecondaryCode) == AsChar
    ...
    local.DelMeLocalCountForPayors.Count = 0;

    foreach(var item in ReadObligorCsePerson())
    {
      if (ReadAdministrativeActCertification2())
      {
        if (!Equal(entities.AdministrativeActCertification.DecertifiedDate,
          local.Null2.Date))
        {
          goto Read1;
        }

        export.ExpotCertifiedSdso.OriginalAmount =
          export.ExpotCertifiedSdso.OriginalAmount.GetValueOrDefault() + entities
          .AdministrativeActCertification.OriginalAmount.GetValueOrDefault();
        export.ExpotCertifiedSdso.CurrentAmount =
          export.ExpotCertifiedSdso.CurrentAmount.GetValueOrDefault() + entities
...
Read1:
      ...
      if (ReadAdministrativeActCertification1())
      {
        ...
        export.ExpotCertifiedFdso.OriginalAmount =
          export.ExpotCertifiedFdso.OriginalAmount.GetValueOrDefault() + entities
          .AdministrativeActCertification.OriginalAmount.GetValueOrDefault();
        export.ExpotCertifiedFdso.CurrentAmount =
          export.ExpotCertifiedFdso.CurrentAmount.GetValueOrDefault() + entities
          .AdministrativeActCertification.CurrentAmount.GetValueOrDefault();
      }
...
Read2:
  ...
  private void UseFnChkUnprcsedTransForCtOrd()
  {
    var useImport = new FnChkUnprcsedTransForCtOrd.Import();
    var useExport = new FnChkUnprcsedTransForCtOrd.Export();
    ...
    useImport.ByStdNum.StandardNumber = import.FilterByStdNo.StandardNumber;
    useImport.OmitCrdInd.Flag = import.IncludeCrdInd.Flag;

    Call(FnChkUnprcsedTransForCtOrd.Execute, useImport, useExport);
  ...
  {
    System.Diagnostics.Debug.Assert(entities.ExistingKeyOnlyObligor.Populated);
    entities.AdministrativeActCertification.Populated = false;

    return Read("ReadAdministrativeActCertification1",
      (db, command) =>
      {
        db.SetString(command, "cpaType", entities.ExistingKeyOnlyObligor.Type1);
        db.SetString(
          command, "cspNumber", entities.ExistingKeyOnlyObligor.CspNumber);
      },
      (db, reader) =>
      {
    ...
    return Read("ReadCashReceiptDetail",
      (db, command) =>
      {
        ...
        db.SetNullableString(
          command, "oblgorPrsnNbr", entities.ExistingKeyOnlyObligor.CspNumber);
      },
      (db, reader) =>
      {
  ...
  private IEnumerable<bool> ReadCollectionObligationTypeObligationObligor()
  {
    ...
    return ReadEach("ReadCollectionObligationTypeObligationObligor",
      (db, command) =>
      {
        db.SetInt32(
      ...
      (db, reader) =>
      {
        ...
        entities.ExistingKeyOnlyObligor.Type1 = db.GetString(reader, 23);
        entities.ExistingObligation.CspNumber = db.GetString(reader, 24);
        entities.ExistingKeyOnlyObligor.CspNumber = db.GetString(reader, 24);
        entities.ExistingObligation.SystemGeneratedIdentifier =
  ...
  private IEnumerable<bool> ReadObligorCsePerson()
  {
    ...
    return ReadEach("ReadObligorCsePerson",
      ...
      (db, reader) =>
      {
        ...
        entities.ExistingObligor.Number = db.GetString(reader, 0);
...
#region Structures
  ...
  {
    /// <summary>
    /// A value of ExpotCertifiedFdso.
    /// </summary>
    [JsonPropertyName("expotCertifiedFdso")]
    public AdministrativeActCertification ExpotCertifiedFdso
    {
      get => expotCertifiedFdso ??= new();
      set => expotCertifiedFdso = value;
    }

    /// <summary>
    /// A value of ExpotCertifiedSdso.
    /// </summary>
    [JsonPropertyName("expotCertifiedSdso")]
    public AdministrativeActCertification ExpotCertifiedSdso
    {
      get => expotCertifiedSdso ??= new();
      set => expotCertifiedSdso = value;
    }
    ...
    private AdministrativeActCertification expotCertifiedFdso;
    private AdministrativeActCertification expotCertifiedSdso;
  ...
  public class Local
  {
    ...
    /// <summary>
    /// A value of DelMeLocalCountForPayors.
    /// </summary>
    [JsonPropertyName("delMeLocalCountForPayors")]
    public Common DelMeLocalCountForPayors
    {
      get => delMeLocalCountForPayors ??= new();
      set => delMeLocalCountForPayors = value;
    }
    ...
    private Common delMeLocalCountForPayors;
    private CsePerson obligor;
