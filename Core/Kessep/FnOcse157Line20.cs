// Program: FN_OCSE157_LINE_20, ID: 371092722, model: 746.
// Short name: SWE02925
using System;
...
/// <summary>
/// A program: FN_OCSE157_LINE_20.
/// </summary>
[Serializable]
public partial class FnOcse157Line20: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_OCSE157_LINE_20 program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnOcse157Line20(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnOcse157Line20.
  /// </summary>
  public FnOcse157Line20(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // 02/04/20  GVandy	CQ66220		Beginning in FY 2022, exclude cases from tribal
    // and
    // 					international child support agencies from Line 20.
    // 					Also, count the CSE case each reporting period
    // 					in which a new incoming case is established.
    // ---------------------------------------------------------------------------------------------------
    ...
    foreach(var item in ReadCase())
    {
        ...
        .Cq66220EffectiveFy.FiscalYear.GetValueOrDefault())
      {
        ...
        //   1) Excluded incoming cases from tribal and foreign country child 
        // support agencies.
        //   2) Count case if an incoming case was established during the 
        // reporting period.
        //      Previously it only counted the first time an incoming case was 
        // established.
        //      So if the CP moves to another state during the reporting period 
        // and we have
        //      to enforce for the new state then we weren't previously counting
        // the case.
        //      Now we will.
        // -------------------------------------------------------------------------------------
        ...
        // 1. Action=R and functional_code = EST, ENF or PAT      OR
        // 2. Action=SPACES, function_code=SPACES and Reason_code=IICNV.
        // -------------------------------------------------------------------------
        foreach(var item1 in ReadInterstateRequest())
        {
          ...
          // --If we get to here then the case is an incoming interstate case 
          // that was
        ...
        //    +-Yes
        //    |   Incoming case created during reporting period?
        //    |       +-Yes
        //    |       |    Case is Incoming from:
        //    |       |        +-Another State
        //    |       |        |   Count the case
        //    |       |        +-Tribal Agency
        ...
        //    |       |        +-
        //    |       +-No
        //    |       |    Message "Skipped-Incoming case not established during
        // report period"
        //    |       +-
        //    +-No
        //    |   Message "Skipped-Case has never been incoming interstate"
        //    +-
        if (AsChar(import.DisplayInd.Flag) == 'Y')
        {
          local.ForCreateOcse157Verification.LineNumber = "20";
          ...
          switch(AsChar(local.InterstateRequestFound.Flag))
          {
            ...
            case 'N':
              local.ForCreateOcse157Verification.Comment =
                "Skipped-Case has never been incoming";
...
Test:
        ...
        .ProgramCheckpointRestart.UpdateFrequencyCount.GetValueOrDefault())
      {
        ...
        local.ProgramCheckpointRestart.RestartInfo = "20 " + entities
          .Case1.Number;
        local.ProgramCheckpointRestart.RestartInfo =
          TrimEnd(local.ProgramCheckpointRestart.RestartInfo) + NumberToString
          (local.Line20Curr.Count, 6, 10);
        local.ProgramCheckpointRestart.RestartInfo =
          TrimEnd(local.ProgramCheckpointRestart.RestartInfo) + NumberToString
          (local.Line20Former.Count, 6, 10);
        local.ProgramCheckpointRestart.RestartInfo =
          TrimEnd(local.ProgramCheckpointRestart.RestartInfo) + NumberToString
          (local.Line20Never.Count, 6, 10);
        UseUpdateCheckpointRstAndCommit();

        if (!IsExitState("ACO_NN0000_ALL_OK"))
        {
          local.ForError.LineNumber = "20";
    ...
    // --------------------------------------------------
    local.ForCreateOcse157Data.LineNumber = "20";
    local.ForCreateOcse157Data.Column = "b";
    local.ForCreateOcse157Data.Number = local.Line20Curr.Count;
    UseFnCreateOcse157Data();
    local.ForCreateOcse157Data.Column = "c";
    local.ForCreateOcse157Data.Number = local.Line20Former.Count;
    UseFnCreateOcse157Data();
    local.ForCreateOcse157Data.Column = "d";
    local.ForCreateOcse157Data.Number = local.Line20Never.Count;
    UseFnCreateOcse157Data();
    local.ProgramCheckpointRestart.RestartInd = "Y";
    local.ProgramCheckpointRestart.RestartInfo = "21 " + " ";
    UseUpdateCheckpointRstAndCommit();
...
#region Structures
  ...
  public class Local
  {
    ...
    /// <summary>
    /// A value of Line20Curr.
    /// </summary>
    [JsonPropertyName("line20Curr")]
    public Common Line20Curr
    {
      get => line20Curr ??= new();
      set => line20Curr = value;
    }

    /// <summary>
    /// A value of Line20Former.
    /// </summary>
    [JsonPropertyName("line20Former")]
    public Common Line20Former
    {
      get => line20Former ??= new();
      set => line20Former = value;
    }

    /// <summary>
    /// A value of Line20Never.
    /// </summary>
    [JsonPropertyName("line20Never")]
    public Common Line20Never
    {
      get => line20Never ??= new();
      set => line20Never = value;
    }
    ...
    private Common line20Curr;
    private Common line20Former;
    private Common line20Never;
