// Program: FN_READ_ONE_DEBT_ACTVTY_FOR_PYR, ID: 374343821, model: 746.
// Short name: SWE01318
using System;
...
/// <summary>
/// <para>
/// A program: FN_READ_ONE_DEBT_ACTVTY_FOR_PYR.
/// </para>
/// <para>
/// RESP: FINANCE
...
/// </summary>
[Serializable]
public partial class FnReadOneDebtActvtyForPyr: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the FN_READ_ONE_DEBT_ACTVTY_FOR_PYR program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new FnReadOneDebtActvtyForPyr(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of FnReadOneDebtActvtyForPyr.
  /// </summary>
  public FnReadOneDebtActvtyForPyr(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  {
    // =================================================
    // 3/24/00 - Bud Adams  -  CREATION
    //   Created by copying RN_Read_Debt_Activity_For_AP_Pyr.
    //   Very frequently the user comes here to see debt activity
    //   for one specific, pre-selected obligation whose identifiers
    //   come to DEBT via a dialog flow.  There was a LOT of
    //   overhead incurred in this case (sorts, retrieved data that
    //   was just tossed aside with Next, etc.) and with Obligation_
    //   Type - which is unnecessary since it is going to be of only
    //   one type.
    // =================================================
    // =================================================
    // RBM 08/01/1997
    ...
    local.MonthEndDate.Date = import.SearchTo.Date;
    ...
    {
      local.MonthStartDate.Date = import.SearchFrom.Date;
    }
    else
    {
      local.MonthStartDate.Date = local.Zero.Date;
    }
    ...
    if (ReadObligation())
    {
      ...
      {
        // =======================================================================
        // In case of secondary Obligation the Summary Totals must be zero.
        //                                                     
        // Vithal Madhira(09/19/00)
        // =======================================================================
        export.ScreenOwedAmounts.TotalAmountOwed = 0;
        export.ScreenOwedAmounts.InterestAmountOwed = 0;
        export.ScreenOwedAmounts.CurrentAmountOwed = 0;
        export.ScreenOwedAmounts.ArrearsAmountOwed = 0;
      }
      else
      {
      }
    ...
    if (AsChar(import.ListDebtsWithAmtOwed.SelectChar) == 'Y')
    {
      ...
      foreach(var item in ReadObligationObligationTransactionDebtDetail1())
      {
    ...
    else
    {
      ...
      //   60th row.
      // =================================================
      foreach(var item in ReadObligationObligationTransactionDebtDetail2())
      {
        if (import.ObligationTransaction.SystemGeneratedIdentifier > 0)
        {
          if (entities.ObligationTransaction.SystemGeneratedIdentifier != import
    ...
    if (!local.Sorted.IsEmpty)
    {
        ...
        local.Sorted.Index)
      {
        ...
        if (AsChar(local.SuppressDebtHeading.Flag) == 'Y')
        {
        }
        else
        {
          ...
          MoveCsePersonsWorkSet2(local.Sorted.Item.
            SupportedSortedCsePersonsWorkSet,
            export.Export1.Update.DtlSupported);
        ...
        foreach(var item in ReadCollection())
        {
          ...
          export.Export1.Update.DtlDebtDetail.DueDt =
            local.Sorted.Item.SortedDebtDetail.DueDt;
          local.CollectionDate.Date = local.InitializedDateWorkArea.Date;
          local.EffectiveDate.Date = local.InitializedDateWorkArea.Date;
          ...
          // 4/7/00 - bud adams  -  PR# 79544: Add Collection
          //   Program_Applied_To to display the Program Code.
          // ===============================================
  ...
  private IEnumerable<bool> ReadObligationObligationTransactionDebtDetail1()
  {
    System.Diagnostics.Debug.Assert(entities.ObligorCsePersonAccount.Populated);
    entities.Obligation.Populated = false;
    ...
    return ReadEach("ReadObligationObligationTransactionDebtDetail1",
      (db, command) =>
      {
        db.
          SetString(command, "cpaType", entities.ObligorCsePersonAccount.Type1);
        ...
        db.SetDate(
          command, "date1", local.MonthStartDate.Date.GetValueOrDefault());
        db.
          SetDate(command, "date2", local.MonthEndDate.Date.GetValueOrDefault());
          
        db.SetDate(
          command, "dueDt", import.NextPageSearchTo.Date.GetValueOrDefault());
        db.SetInt32(
          command, "obTrnId",
  ...
  private IEnumerable<bool> ReadObligationObligationTransactionDebtDetail2()
  {
    System.Diagnostics.Debug.Assert(entities.ObligorCsePersonAccount.Populated);
    entities.Obligation.Populated = false;
    entities.ObligationTransaction.Populated = false;
    ...
    return ReadEach("ReadObligationObligationTransactionDebtDetail2",
      (db, command) =>
      {
        db.
          SetString(command, "cpaType", entities.ObligorCsePersonAccount.Type1);
  ...
  private IEnumerable<bool> ReadObligationTransactionRln()
  {
    ...
    return ReadEach("ReadObligationTransactionRln",
      (db, command) =>
      {
        ...
        db.SetDateTime(
          command, "createdTmst",
          export.NextPage.CreatedTmst.GetValueOrDefault());
      },
      (db, reader) =>
      {
...
#region Structures
  ...
  public class Export
  {
    ...
    public ObligationTransactionRln NextPage
    {
      get => nextPage ??= new();
      set => nextPage = value;
    }
    ...
    private ObligationTransactionRln nextPage;
  }
  ...
  public class Local
  {
    ...
    /// <summary>
    /// A value of InitializedCase.
    /// </summary>
    [JsonPropertyName("initializedCase")]
    public Case1 InitializedCase
    {
      get => initializedCase ??= new();
      set => initializedCase = value;
    }
