// Program: OE_B467_HOUSEKEEPING, ID: 374472705, model: 746.
// Short name: SWE02708
using System;
...
/// <summary>
/// A program: OE_B467_HOUSEKEEPING.
/// </summary>
[Serializable]
public partial class OeB467Housekeeping: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the OE_B467_HOUSEKEEPING program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new OeB467Housekeeping(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of OeB467Housekeeping.
  /// </summary>
  public OeB467Housekeeping(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // 00-06-29  PR 86861  Fangman           Initial code
    // ---------------------------------------------
    // ***** Get the run parameters for this program.
    export.ProgramProcessingInfo.Name = "SWEEB467";
    UseReadProgramProcessingInfo();

    if (IsExitState("ACO_NN0000_ALL_OK"))
    {
      export.SetTriggers.Flag =
      ...
      {
        export.Test.TestFirstSup.Number =
          TrimEnd(Substring(export.ProgramProcessingInfo.ParameterList, 7, 10));
      ...
      {
        export.Test.TestLastSup.Number =
          TrimEnd(Substring(export.ProgramProcessingInfo.ParameterList, 18, 10));
    ...
    if (AsChar(export.SetTriggers.Flag) == 'Y')
    {
      local.EabReportSend.RptDetail = "Trigger flag set to " + export
        .SetTriggers.Flag + " triggers will be set.";
    }
    else
    {
      local.EabReportSend.RptDetail = "Trigger flag set to " + export
        .SetTriggers.Flag + " triggers will not be set.";
    }
    ...
    if (!IsEmpty(export.Test.TestFirstSup.Number) || !
      IsEmpty(export.Test.TestLastSup.Number))
    {
      local.EabReportSend.RptDetail =
        "Deriving Foster Care URA for supported persons between: " + export
        .Test.TestFirstSup.Number + " and " + export.Test.TestLastSup.Number;
      UseCabControlReport1();
    ...
    if (AsChar(export.Test.TestRunInd.Flag) == 'Y' || AsChar
      ...
      IsEmpty(export.Test.TestFirstSup.Number) || !
      IsEmpty(export.Test.TestLastSup.Number))
    {
      local.EabReportSend.RptDetail = "";
      UseCabControlReport1();
