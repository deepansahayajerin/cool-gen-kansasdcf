...
/// <summary>
/// <para>
/// A program: OE_WOR2_CS_WORKSHEET_PAGE_2.
/// </para>
/// <para>
/// Resp - OBLGESTB
/// </para>
...
public partial class OeWor2CsWorksheetPage2: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the OE_WOR2_CS_WORKSHEET_PAGE_2 program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new OeWor2CsWorksheetPage2(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of OeWor2CsWorksheetPage2.
  /// </summary>
  public OeWor2CsWorksheetPage2(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // 					edits.
    // 11/06/19  GVandy	CQ66067		2020 Guideline changes.
    // 01/09/20  GVandy	CQ67085		Apply Parenting Time Adjustment to NCP only.
    // 					Automagically calculate the Credit for
    // 					Insurance and Work Related Child Care.
    // ---------------------------------------------------------------
    ...
    // @@@ changes below...
    // 01/09/20 GVandy  CQ67085  Apply Parenting Time Adjustment to NCP only.
    // --The Parent to which the Parenting Time Adjustment applies was 
    // previously user
    //   enterable.  Now we automatically apply the adjustment to the NCP.
    if (export.Gexport2020EnterableFields.GexportParentingTime.TotalCurrency ==
      0)
    {
      export.Gexport2020EnterableFields.GexportParentingTime.SelectChar = "";
    }
    else if (export.Gexport2020EnterableFields.GexportParentingTime.
      TotalCurrency > 100)
    {
      var field =
        GetField(export.Gexport2020EnterableFields.GexportParentingTime,
        "totalCurrency");
    ...
    else
    {
      ...
      if (AsChar(local.Parent1.SelectChar) == AsChar(local.Parent2.SelectChar))
      {
      ...
      else
      {
        ...
        if (IsEmpty(export.Gexport2020EnterableFields.GexportParentingTime.
          SelectChar))
        {
