...
/// <summary>
/// <para>
/// A program: OE_WXR2_PREV_YR_CS_WS_PAGE_2.
/// </para>
/// <para>
/// Resp - OBLGESTB
/// </para>
...
public partial class OeWxr2PrevYrCsWsPage2: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the OE_WXR2_PREV_YR_CS_WS_PAGE_2 program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new OeWxr2PrevYrCsWsPage2(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of OeWxr2PrevYrCsWsPage2.
  /// </summary>
  public OeWxr2PrevYrCsWsPage2(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // from 7 to 6.
    // 03/20/12 A Hockman      SR# 16297       Changes to allow for keeping 
    // multiple years
    //                                         
    // of guidelines. Added display of GL year
    // on
    //                                         
    // screen.
    // 12/09/15 GVandy		CQ50299		Add edit that KS Tax Credit must be "N"
    // 					starting with guideline year 2016.
    // 					Restructured Work Related Child Care Cost
    // 					edits.
    // ---------------------------------------------------------------
    ...
    switch(TrimEnd(global.Command))
    {
      ...
      case "HELP":
        ...
        break;
      case "PREV":
        // 12/09/2015 GVandy CQ50299 Restructured Work Related Child Care Cost 
        // edits.
        // -- Child Care Cost Defaults and Validation for Parent B
        if (import.ParentBCsePersonSupportWorksheet.WorkRelatedChildCareCosts.
          GetValueOrDefault() == 0 && import
          .ParentBCsePersonSupportWorksheet.NoOfChildrenInDayCare.
            GetValueOrDefault() == 0)
        {
        ...
        {
          // -- Default/Validate Kansas Tax Credit indicator
          switch(AsChar(import.ParentBCsePersonSupportWorksheet.
            EligibleForKansasTaxCredit))
          {
            case ' ':
              if (export.ChildSupportWorksheet.CsGuidelineYear < 2016)
              {
                export.ParentBCsePersonSupportWorksheet.
                  EligibleForKansasTaxCredit = "Y";
              }
              else
              {
                export.ParentBCsePersonSupportWorksheet.
                  EligibleForKansasTaxCredit = "N";
              }
            ...
            case 'Y':
              ...
              else
              {
                ...
                ExitState = "OE0000_INVALID_KS_TAX_INDICATOR";
              }
          ...
          // -- Default/Validate Federal Tax Credit indicator
          switch(AsChar(import.ParentBCsePersonSupportWorksheet.
          ...
          // -- Validate the Number of Children in Daycare
          ...
          // -- Validate the Actual Work Related Child Care Cost
        ...
        // -- Child Care Cost Defaults and Validation for Parent A
        if (import.ParentACsePersonSupportWorksheet.WorkRelatedChildCareCosts.
          GetValueOrDefault() == 0 && import
          .ParentACsePersonSupportWorksheet.NoOfChildrenInDayCare.
            GetValueOrDefault() == 0)
        {
        ...
        {
          // -- Default/Validate Kansas Tax Credit indicator
          switch(AsChar(import.ParentACsePersonSupportWorksheet.
            EligibleForKansasTaxCredit))
          {
            case ' ':
              if (export.ChildSupportWorksheet.CsGuidelineYear < 2016)
        ...
        // -------------------------------------------------------------------------------------
        // --- Original Work Related Child Care Cost edits below.
        // -------------------------------------------------------------------------------------
        if (import.ParentACsePersonSupportWorksheet.NoOfChildrenInDayCare.
