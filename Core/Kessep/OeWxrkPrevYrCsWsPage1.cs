...
/// <summary>
/// <para>
/// A program: OE_WXRK_PREV_YR_CS_WS_PAGE_1.
/// </para>
/// <para>
/// Resp - OBLGESTB
/// </para>
...
public partial class OeWxrkPrevYrCsWsPage1: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the OE_WXRK_PREV_YR_CS_WS_PAGE_1 program.
  /// </summary>
  [Entry]
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new OeWxrkPrevYrCsWsPage1(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of OeWxrkPrevYrCsWsPage1.
  /// </summary>
  public OeWxrkPrevYrCsWsPage1(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    if (Equal(global.Command, "CLEAR") || Equal(global.Command, "RDISPLAY") || Equal
    ...
    if (Equal(global.Command, "RETCSWL"))
    {
      ...
      {
        // -- Return to the WORK screen for worksheets prior to 2020.  The work 
    ...
    switch(TrimEnd(global.Command))
    {
      case "RETCDVL":
        ...
        if (AsChar(export.GuidelineYearPrompt.SelectChar) == 'S')
        {
          ...
          if (!IsEmpty(import.SelectedCodeValue.Cdvalue))
          {
            ...
            if (export.ChildSupportWorksheet.CsGuidelineYear >= 2020)
            {
              // -- Return to the WORK screen for worksheets prior to 2020.  The
              // work sheet layout and calculations were changed starting in
              // 2020.
              global.Command = "DISPPAG2";
              ExitState = "ACO_NE0000_RETURN";

              // --Remove the parenting time adjustment from the "adjustment" 
              // section.
                ...
                .Export1.Count; ++export.Export1.Index)
              {
                ...
                else
                {
                  ...
                  local.Local1.Update.Work.SelectChar =
                    export.Export1.Item.Work.SelectChar;
                  local.Local1.Update.ParentA.AdjustmentAmount =
                    export.Export1.Item.ParentA.AdjustmentAmount;
                  local.Local1.Update.ParentB.AdjustmentAmount =
                    export.Export1.Item.ParentB.AdjustmentAmount;
                }
              }
              ...
              while(local.Local1.Index + 1 != Local.LocalGroup.Capacity);
      ...
      case "PRINT":
        ...
        // --CQ66067 Use prior year worksheet document.
        local.Document.Name = "WRKSHPYR";
      ...
      case "CREATE":
          ...
          (import.ChildSupportWorksheet.MultipleFamilyAdjInd) == 'N')
        {
            ...
            GetValueOrDefault() > 0)
          {
            ...
            ExitState = "OE0175_WORKSHEET_FAMILY_ADJ";
          }
        }
          ...
          )
        {
          ...
          if (IsExitState("ACO_NN0000_ALL_OK"))
          {
            ExitState = "OE0000_NO_OF_ADD_CHILDREN_REQD";
          }
        }
...
Test1:
        ...
        if (IsExitState("ACO_NI0000_SUCCESSFUL_ADD"))
        {
          ...
          export.PrevParentACsePerson.Number = export.ParentACsePerson.Number;
          export.PrevParentBCsePerson.Number = export.ParentBCsePerson.Number;
          MoveCsePersonSupportWorksheet3(export.
          ...
          if (export.ChildSupportWorksheet.CsGuidelineYear != local
            .CurrentGuidelineYear.Year)
          {
            ExitState = "SI000_ADD_SUCCESSFUL_PRIOR_YR_GL";
          }
...
Test2:
    ...
    if (Equal(global.Command, "DISPLAY"))
    {
      ...
      if (IsExitState("ACO_NI0000_SUCCESSFUL_DISPLAY"))
      {
        ...
        export.PrevParentACsePerson.Number = export.ParentACsePerson.Number;
        export.PrevParentBCsePerson.Number = export.ParentBCsePerson.Number;
        MoveCsePersonSupportWorksheet3(export.ParentACsePersonSupportWorksheet,
          export.PrevParentACsePersonSupportWorksheet);
        MoveCsePersonSupportWorksheet3(export.ParentBCsePersonSupportWorksheet,
          export.PrevParentBCsePersonSupportWorksheet);
  ...
  private static void MoveCsePersonSupportWorksheet3(
    CsePersonSupportWorksheet source, CsePersonSupportWorksheet target)
  {
    target.WageEarnerGrossIncome = source.WageEarnerGrossIncome;
    target.SelfEmploymentGrossIncome = source.SelfEmploymentGrossIncome;
  ...
  private void UseOeWorkAddChildSupWorksheet()
  {
    ...
    MoveCsePersonSupportWorksheet2(export.ParentBCsePersonSupportWorksheet,
      useImport.ParentBCsePersonSupportWorksheet);
    MoveCsePersonSupportWorksheet2(export.ParentACsePersonSupportWorksheet,
      useImport.ParentACsePersonSupportWorksheet);
  ...
  private void UseOeWorkUpdtChildSupWorksheet()
  {
    ...
    MoveCsePersonSupportWorksheet2(import.ParentBCsePersonSupportWorksheet,
      useImport.ParentBCsePersonSupportWorksheet);
    MoveCsePersonSupportWorksheet2(import.ParentACsePersonSupportWorksheet,
      useImport.ParentACsePersonSupportWorksheet);
