// Program: SI_CREATE_IS_REQUEST, ID: 372390941, model: 746.
// Short name: SWE01138
using System;
...
/// <summary>
/// <para>
/// A program: SI_CREATE_IS_REQUEST.
/// </para>
/// <para>
/// RESP: SRVINIT
...
/// </summary>
[Serializable]
public partial class SiCreateIsRequest: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the SI_CREATE_IS_REQUEST program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new SiCreateIsRequest(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of SiCreateIsRequest.
  /// </summary>
  public SiCreateIsRequest(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // ---------------------------------------------
    // 03/19/99 W.Campbell        Added a
    //                            local timestamp view and
    //                            modified statements to use it.
    //                            Also added PV and AE exit states.
    // -----------------------------------------
    // 06/07/99 W.Campbell        Changed a
    //                            set stmt to initialize the
    //                            interstate_request
    //                            other_state_case_status
    //                            to "O" instead of spaces.
    // -----------------------------------------
    // 06/11/99 W.Campbell        Disabled
    //                            set stmts to initialize
    //                            local interstate_request
    ...
    //                            other_state_case_closure_date
    //                            so that it will be set from
    //                            an import view.  This is inside
    //                            the create stmt for
    //                            interstate_request.
    // -----------------------------------------
    ...
    // ---------------------------------------------
    // If the referral is  Initiated from KS state,
    // then the Interstate_Case # would not be present.
    // Set the KS Case Ind to "Y".
    // The other state case details should be spaces.
    // 4/29/97	  SHERAZ MALIK 		CHANGE CURRENT_DATE
    // For a out-of-state Case, Set the KS Case Ind to "N".
    // ---------------------------------------------
    // --------------------------------------------------------------------------------
    // If this is an CSI CSNET transaction from OINR, The KS_IND
    // will be set to spaces.
    // 07/11/01  Tom Bobb PR 00123030
    // --------------------------------------------------------------------------------
    if (IsEmpty(import.InterstateRequest.KsCaseInd))
    {
      local.InterstateRequest.KsCaseInd = import.InterstateRequest.KsCaseInd;
    }
    else if (IsEmpty(import.Incoming.InterstateCaseId))
    {
      ...
      // -----------------------------------------
      // 06/07/99 W.Campbell - Changed the following
      // set stmt to initialize the interstate_request
      // other_state_case_status to "O" instead of
      // spaces.
      // -----------------------------------------
      ...
      // -----------------------------------------
      // 06/11/99 W.Campbell - Disabled
      // set stmt to initialize local interstate_request
      // other_state_case_closure_date to datnum(0).
      // -----------------------------------------
    }
    else
    {
      ...
      // *************************************************************
      // 03/20/00   C. Ott   PR # 85011.   Need to assure that Case Type 
      // attribute is not empty for outgoing transaction.  If imported value is
      // spaces, call action blocks to derive active case type or arrears case
      // type.
      // *************************************************************
