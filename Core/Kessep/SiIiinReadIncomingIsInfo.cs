// Program: SI_IIIN_READ_INCOMING_IS_INFO, ID: 372500276, model: 746.
// Short name: SWE01224
using System;
...
/// <summary>
/// <para>
/// A program: SI_IIIN_READ_INCOMING_IS_INFO.
/// </para>
/// <para>
/// RESP: SRVINIT
/// </para>
/// </summary>
[Serializable]
public partial class SiIiinReadIncomingIsInfo: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the SI_IIIN_READ_INCOMING_IS_INFO program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new SiIiinReadIncomingIsInfo(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of SiIiinReadIncomingIsInfo.
  /// </summary>
  public SiIiinReadIncomingIsInfo(IContext context, Import import, Export export)
    :
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    ...
    // 12/28/98    C Deghand         Added an Exit State and an ESCAPE
    //                               
    // when there is no AP.
    // 12/29/98    C Deghand         Added code to check for multiple
    //                               
    // Interstate Requests.
    // 2/17/99     C Deghand         Added code to check for country
    //                               
    // and state FIPS in Interstate
    // Request.
    // -----------------------------------------------------------
    ...
    // 12/29/98  C Deghand  Copied the code to check for multiple interstate 
    // requests from IIMC.
    if (IsEmpty(import.OtherState.StateAbbreviation))
    {
      ...
      else if (local.StateCount.Count == 0)
      {
        if (local.ForeignCount.Count == 0)
        {
          return;
    ...
    if (ReadInterstateRequest1())
    {
      ...
      if (AsChar(export.InterstateRequest.OtherStateCaseStatus) == 'O')
      {
        export.InterstateRequest.OtherStateCaseClosureDate = local.Refresh.Date;
      }
    ...
    if (ReadFips1())
    {
      export.OtherState.Assign(entities.Fips);
    }
