// Program: SP_ALRT_SEARCH_BY_SERVICE_PRVDER, ID: 1902640511, model: 746.
// Short name: SWE01344
using System;
...
/// <summary>
/// A program: SP_ALRT_SEARCH_BY_SERVICE_PRVDER.
/// </summary>
[Serializable]
public partial class SpAlrtSearchByServicePrvder: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the SP_ALRT_SEARCH_BY_SERVICE_PRVDER program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new SpAlrtSearchByServicePrvder(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of SpAlrtSearchByServicePrvder.
  /// </summary>
  public SpAlrtSearchByServicePrvder(IContext context, Import import,
    Export export):
    base(context)
  {
    this.import = import;
...
#region Implementation
  ...
  public void Run()
  {
    // CQ56117  Search by Service Provider ID
    ...
    switch(TrimEnd(import.SortDesc.Cdvalue))
    {
      case "DA":
        ...
        foreach(var item in ReadOfficeServiceProviderAlert1())
        {
          ...
          MoveInfrastructure(local.InitialInfrastr, local.Work);
          ...
          if (ReadInfrastructure())
          {
            ...
            {
              if (!Equal(entities.Infrastructure.EventType,
                local.Filter.EventType))
              {
                continue;
              }
            }
            ...
            if (!IsEmpty(local.Filter.DenormText12))
            {
              ...
              {
                // --The court order number does not match the import filter 
                // value requested.
                continue;
              }
                    ...
                    switch(Length(TrimEnd(local.Fips.Text4)))
                    {
                      case 2:
                        ...
                        if (!IsEmpty(import.FilterFipsTribAddress.Country) && Equal
                          (import.FilterFipsTribAddress.Country,
                          local.Fips.Text4, 1, 2))
                        {
                          // --The tribunal matches the filter.  Continue.
                        }
                        else
                        {
                          // --The tribunal does not match the filter.  Skip 
                          // this alert.
                          continue;
                        }
                      ...
                      case 4:
                        ...
                        if (!IsEmpty(import.FilterFips.StateAbbreviation) && Equal
                          (import.FilterFips.StateAbbreviation,
                          local.Fips.Text4, 1, 2) && !
                          IsEmpty(import.FilterFips.CountyAbbreviation) && Equal
                          (import.FilterFips.CountyAbbreviation,
                          local.Fips.Text4, 3, 2))
                        {
                          // --The tribunal matches the filter.  Continue.
                        }
                        else
            ...
            // ****** Move to group view only if we found what we are looking 
            // for.
            local.Work.Assign(entities.Infrastructure);

            ++export.Export1.Index;
            export.Export1.CheckSize();

            // *****Move to group view onl if we found what we are looking for.
            ...
            export.Export1.Update.GxegalAction.CourtCaseNumber =
              local.La.CourtCaseNumber;
            export.Export1.Update.Gxnfrastructure.Assign(local.Work);
      ...
      case "DD":
        ...
        foreach(var item in ReadOfficeServiceProviderAlert3())
        {
          if (!IsEmpty(import.FilterOfficeServiceProviderAlert.Message))
          {
            if (!Equal(entities.OfficeServiceProviderAlert.Message,
      ...
      case "CN":
        ...
        foreach(var item in ReadOfficeServiceProviderAlertInfrastructure1())
        {
          ...
          // ****** Move to group view only if we found what we are looking for.
          local.Work.Assign(entities.Infrastructure);
  ...
  private IEnumerable<bool> ReadOfficeServiceProviderAlert1()
  {
    ...
    return ReadEach("ReadOfficeServiceProviderAlert1",
      (db, command) =>
      {
        ...
        db.SetInt32(
          command, "systemGeneratedId", import.FilterOffice.SystemGeneratedId);
        db.SetString(
          command, "roleCode", import.FilterOfficeServiceProvider.RoleCode);
        db.SetDate(
          command, "effectiveDate",
          import.FilterOfficeServiceProvider.EffectiveDate.GetValueOrDefault());
      ...
      (db, reader) =>
      {
        ...
        entities.OfficeServiceProviderAlert.CreatedBy = db.GetString(reader, 8);
  ...
  private IEnumerable<bool> ReadOfficeServiceProviderAlert3()
  {
    entities.OfficeServiceProviderAlert.Populated = false;

    return ReadEach("ReadOfficeServiceProviderAlert3",
      (db, command) =>
      {
        db.SetDate(
          command, "distributionDate1",
  ...
  private IEnumerable<bool> ReadOfficeServiceProviderAlertInfrastructure1()
  {
    ...
    return ReadEach("ReadOfficeServiceProviderAlertInfrastructure1",
      ...
      (db, reader) =>
      {
        ...
        entities.OfficeServiceProviderAlert.OspDate =
          db.GetNullableDate(reader, 14);
        entities.Infrastructure.ProcessStatus = db.GetString(reader, 15);
        entities.Infrastructure.EventId = db.GetInt32(reader, 16);
        entities.Infrastructure.EventType = db.GetString(reader, 17);
        entities.Infrastructure.EventDetailName = db.GetString(reader, 18);
        entities.Infrastructure.ReasonCode = db.GetString(reader, 19);
        entities.Infrastructure.BusinessObjectCd = db.GetString(reader, 20);
        entities.Infrastructure.DenormNumeric12 =
          db.GetNullableInt64(reader, 21);
        entities.Infrastructure.DenormText12 = db.GetNullableString(reader, 22);
        entities.Infrastructure.CaseNumber = db.GetNullableString(reader, 23);
        entities.Infrastructure.CsePersonNumber =
          db.GetNullableString(reader, 24);
        entities.Infrastructure.CaseUnitNumber =
          db.GetNullableInt32(reader, 25);
        entities.Infrastructure.Detail = db.GetNullableString(reader, 26);
        entities.Infrastructure.Populated = true;
        entities.OfficeServiceProviderAlert.Populated = true;
