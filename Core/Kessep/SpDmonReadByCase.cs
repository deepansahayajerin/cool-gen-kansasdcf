// Program: SP_DMON_READ_BY_CASE, ID: 372916832, model: 746.
// Short name: SWE01801
using System;
...
/// <summary>
/// <para>
/// A program: SP_DMON_READ_BY_CASE.
/// </para>
/// <para>
...
/// </summary>
[Serializable]
public partial class SpDmonReadByCase: Bphx.Cool.Action
{
  /// <summary>
  /// Executes the SP_DMON_READ_BY_CASE program.
  /// </summary>
  public static readonly Action<IContext, Import, Export> Execute =
    (c, i, e) => new SpDmonReadByCase(c, i, e).Run();

  /// <summary>
  /// Constructs an instance of SpDmonReadByCase.
  /// </summary>
  public SpDmonReadByCase(IContext context, Import import, Export export):
    base(context)
  {
    this.import = import;
    this.export = export;
...
#region Implementation
  ...
  {
    // -----------------------------------------------------------------------------------
    // This AB reads monitored documents that are for a specific CASE
    // -----------------------------------------------------------------------------------
    ...
    if (!IsEmpty(import.FilterInfrastructure.CsePersonNumber))
    {
      ...
      if (AsChar(import.FilterShowAll.Text1) == 'Y')
      {
        ...
        foreach(var item in ReadMonitoredDocumentInfrastructure3())
        {
          ...
          export.Export1.Update.GxineInfrastructure.Assign(
    ...
    else if (AsChar(import.FilterShowAll.Text1) == 'Y')
    {
      export.Export1.Index = 0;
      export.Export1.Clear();

      foreach(var item in ReadMonitoredDocumentInfrastructure4())
      {
        MoveInfrastructure(entities.IndexOnly, local.Infrastructure);
  ...
  private IEnumerable<bool> ReadMonitoredDocumentInfrastructure1()
  {
    return ReadEach("ReadMonitoredDocumentInfrastructure1",
      ...
      (db, reader) =>
      {
        ...
        entities.IndexOnly.CaseNumber = db.GetNullableString(reader, 6);
        entities.IndexOnly.CsePersonNumber = db.GetNullableString(reader, 7);
        entities.IndexOnly.UserId = db.GetString(reader, 8);
  ...
  private IEnumerable<bool> ReadMonitoredDocumentInfrastructure3()
  {
    return ReadEach("ReadMonitoredDocumentInfrastructure3",
      (db, command) =>
      {
        db.SetDate(
          command, "requiredResponse",
  ...
  private IEnumerable<bool> ReadMonitoredDocumentInfrastructure4()
  {
    return ReadEach("ReadMonitoredDocumentInfrastructure4",
      (db, command) =>
      {
        db.SetDate(
          command, "requiredResponse",
